...
func (e *ProxyAgentError) Error() string { return e.Message }
...
func ResolvePublicTarget(ctx context.Context, raw string, config Config) (*resolvedTarget, error) {
	...
	if err != nil || target.Hostname() == "" {
		return nil, &ProxyAgentError{"INVALID_TARGET_URL", "spec_url must be a complete HTTP or HTTPS URL.", http.StatusBadRequest}
	}
	if target.Scheme != "http" && target.Scheme != "https" {
		return nil, &ProxyAgentError{"TARGET_PROTOCOL_BLOCKED", "Only HTTP and HTTPS targets are allowed.", http.StatusBadRequest}
	}
	if target.User != nil {
		return nil, &ProxyAgentError{"TARGET_CREDENTIALS_BLOCKED", "Target credentials in URLs are not allowed.", http.StatusBadRequest}
	}
	host := strings.ToLower(target.Hostname())
	if host == "localhost" || strings.HasSuffix(host, ".localhost") || strings.HasSuffix(host, ".local") {
		return nil, &ProxyAgentError{"TARGET_HOST_BLOCKED", "Local hostnames are blocked.", http.StatusForbidden}
	}
	if !hostAllowed(host, config.AllowedHosts) {
		return nil, &ProxyAgentError{"TARGET_HOST_NOT_ALLOWED", "The target host is not in OPENDOC_ALLOWED_REMOTE_HOSTS.", http.StatusForbidden}
	}
	port := 80
	...
	if target.Port() != "" {
		...
		if err != nil {
			return nil, &ProxyAgentError{"INVALID_TARGET_URL", "The target port is invalid.", http.StatusBadRequest}
		}
	}
	if !config.AllowedPorts[port] {
		return nil, &ProxyAgentError{"TARGET_PORT_BLOCKED", fmt.Sprintf("Remote port %d is not allowed.", port), http.StatusForbidden}
	}
	addresses, err := net.DefaultResolver.LookupIP(ctx, "ip", host)
	if err != nil || len(addresses) == 0 {
		return nil, &ProxyAgentError{"TARGET_DNS_FAILED", "The target hostname could not be resolved.", http.StatusBadGateway}
	}
	for _, address := range addresses {
		if !IsPublicIP(address) {
			return nil, &ProxyAgentError{"TARGET_ADDRESS_BLOCKED", "The target resolves to a private, reserved, or otherwise prohibited address.", http.StatusForbidden}
		}
	}
...
func requestOnce(ctx context.Context, raw string, incoming http.Header, config Config) (*http.Response, error) {
	...
	request.Header.Set("User-Agent", "OpenDoc-Proxy-Agent/0.1")
	...
	if err != nil {
		...
		return nil, &ProxyAgentError{"REMOTE_CONNECTION_FAILED", "The remote server could not be reached.", http.StatusBadGateway}
	}
	...
	for redirects := 0; redirects <= config.MaxRedirects; redirects++ {
		...
		if response.StatusCode >= 300 && response.StatusCode <= 399 && response.Header.Get("Location") != "" {
			...
			if redirects == config.MaxRedirects {
				return nil, &ProxyAgentError{"REMOTE_REDIRECT_LIMIT", "Remote redirect limit exceeded.", http.StatusBadGateway}
			}
			base, _ := url.Parse(current)
			location, err := base.Parse(response.Header.Get("Location"))
			if err != nil {
				return nil, &ProxyAgentError{"REMOTE_REDIRECT_INVALID", "Remote redirect URL is invalid.", http.StatusBadGateway}
			}
		...
		if response.StatusCode < 200 || response.StatusCode >= 300 {
			response.Body.Close()
			return nil, &ProxyAgentError{"REMOTE_HTTP_STATUS", fmt.Sprintf("Remote server returned HTTP %d.", response.StatusCode), http.StatusBadGateway}
		}
		if response.ContentLength > config.MaxBytes {
			response.Body.Close()
			return nil, &ProxyAgentError{"REMOTE_FILE_TOO_LARGE", "Remote specification exceeds OPENDOC_MAX_BYTES.", http.StatusRequestEntityTooLarge}
		}
		body, err := io.ReadAll(io.LimitReader(response.Body, config.MaxBytes+1))
		response.Body.Close()
		if err != nil {
			return nil, &ProxyAgentError{"REMOTE_READ_FAILED", "The remote response could not be read.", http.StatusBadGateway}
		}
...
func requestOnceProxied(ctx context.Context, raw string, method string, headers map[string]string, body []byte, config Config) (*http.Response, error) {
	...
	if err != nil {
		transport.CloseIdleConnections()
		return nil, &ProxyAgentError{"INVALID_TARGET_METHOD", "Target method is not valid.", http.StatusBadRequest}
	}
	request.Header.Set("Accept-Encoding", "identity")
	request.Header.Set("User-Agent", "OpenDoc-Proxy-Agent/0.1")
...
func ExecuteProxiedRequest(ctx context.Context, rawBody []byte, incoming http.Header, config Config) (*ProxiedResponse, error) {
	...
	if !config.ProxyEnabled {
		return nil, &ProxyAgentError{"PROXY_DISABLED", "The request proxy is disabled on this service.", http.StatusForbidden}
	}
	target := strings.TrimSpace(incoming.Get("X-OpenDoc-Target-Url"))
	if target == "" {
		return nil, &ProxyAgentError{"MISSING_TARGET_URL", "Missing X-OpenDoc-Target-Url header.", http.StatusBadRequest}
	}
	...
	if raw := strings.TrimSpace(incoming.Get("X-OpenDoc-Target-Headers")); raw != "" {
		...
		if err := json.Unmarshal([]byte(raw), &parsedHeaders); err != nil {
			return nil, &ProxyAgentError{"INVALID_TARGET_HEADERS", "X-OpenDoc-Target-Headers must be a JSON object.", http.StatusBadRequest}
		}
	...
	for redirects := 0; redirects <= config.MaxRedirects; redirects++ {
		...
		if location != "" && isRedirect(response.StatusCode) && redirects < config.MaxRedirects {
			...
			if err != nil {
				return nil, &ProxyAgentError{"REMOTE_REDIRECT_TARGET", "Remote redirect location is not valid.", http.StatusBadGateway}
			}
		...
		if int64(len(payload)) > config.MaxBytes {
			return nil, &ProxyAgentError{"REMOTE_RESPONSE_TOO_LARGE", "Remote response exceeds OPENDOC_MAX_BYTES.", http.StatusBadGateway}
		}
...
func writeError(writer http.ResponseWriter, err error) {
	failure := &ProxyAgentError{"PROXY_AGENT_ERROR", "Proxy agent request failed.", http.StatusBadGateway}
...
func (handler *Handler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	if request.URL.Path != "/download" {
		writeError(writer, &ProxyAgentError{"NOT_FOUND", "Route not found.", http.StatusNotFound})
		return
	}
	if !handler.applyCORS(writer, request) {
		writeError(writer, &ProxyAgentError{"ORIGIN_NOT_ALLOWED", "Browser origin is not allowed.", http.StatusForbidden})
		return
	}
	...
	if request.Method != http.MethodGet {
		writeError(writer, &ProxyAgentError{"METHOD_NOT_ALLOWED", "Only GET and OPTIONS are allowed.", http.StatusMethodNotAllowed})
		return
	}
	...
	if !handler.rateAllowed(client) {
		writeError(writer, &ProxyAgentError{"RATE_LIMITED", "Proxy agent rate limit exceeded.", http.StatusTooManyRequests})
		return
	}
	target := request.URL.Query().Get("spec_url")
	if target == "" {
		writeError(writer, &ProxyAgentError{"MISSING_TARGET_URL", "Missing spec_url query parameter.", http.StatusBadRequest})
		return
	}
...
func (handler *Handler) serveProxy(writer http.ResponseWriter, request *http.Request) {
	...
	if request.Method != http.MethodPost {
		writeError(writer, &ProxyAgentError{"METHOD_NOT_ALLOWED", "Only POST and OPTIONS are allowed.", http.StatusMethodNotAllowed})
		return
	}
	client := request.Header.Get("X-Forwarded-For")
	if client == "" {
	...
	client = strings.TrimSpace(strings.Split(client, ",")[0])
	if !handler.rateAllowed(client) {
		writeError(writer, &ProxyAgentError{"RATE_LIMITED", "Proxy rate limit exceeded.", http.StatusTooManyRequests})
		return
	}
	request.Body = http.MaxBytesReader(writer, request.Body, handler.Config.MaxBytes)
	rawBody, err := io.ReadAll(request.Body)
	if err != nil {
		writeError(writer, &ProxyAgentError{"REQUEST_BODY_TOO_LARGE", "Proxied request body exceeds OPENDOC_MAX_BYTES.", http.StatusRequestEntityTooLarge})
		return
	}
