...
assert.equal(operation.requestBody.content['application/x-www-form-urlencoded'].encoding.tags['x-opendoc-collection-format'], 'pipes');
const auth = {
    ...createEmptyAuth(), activeScheme: 'api_key', selectedSchemes: ['api_key'], requirementIndex: 0,
    schemeValues: {api_key: {schemeId: 'api_key', type: 'apiKey' as const, name: 'X-API-Key', in: 'header' as const, value: 'secret'}},
};
const plan = compileBrowserRequest({
    spec, path: '/items/{id}', method: 'post', operation,
    selectedServer: spec.servers[0].url, activeAuth: auth,
    parameterValues: {
...
const rawPlan = compileBrowserRequest({
    spec, path: '/raw', method: 'post', operation: rawOperation,
    selectedServer: spec.servers[0].url, activeAuth: createEmptyAuth(),
    body: 'deliberately untyped', bodyType: '*/*',
});
...
const noHost: any = normalizeOpenApiSpec({
    swagger: '2.0', info: {title: 'Relative', version: '1'}, basePath: '/api', paths: {},
});
assert.deepEqual(noHost.servers, [{url: '/api'}]);
const protocolRelative: any = normalizeOpenApiSpec({
    swagger: '2.0', info: {title: 'Protocol', version: '1'}, host: 'api.test', paths: {},
});
