...
export const resolveGatewaySelection = (
    ...
    body: unknown,
): {provider: AIProviderId; model: string} | {error: string} => {
    ...
    if (!policy.allowedModels.has(requestedModel)) return {error: `Model '${requestedModel}' is not allowed by this gateway.`};
