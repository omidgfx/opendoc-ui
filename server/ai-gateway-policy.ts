...
const SUPPORTED_PROVIDERS = new Set<AIProviderId>([
    'openrouter',
    'ollama',
    'openai',
    'anthropic',
...
const exactList = (raw: string): string[] => raw.split(',').map(value => value.trim()).filter(Boolean);
export const createGatewayModelPolicy = (input: {
    ...
    if (!SUPPORTED_PROVIDERS.has(provider))
        throw new Error(`Unsupported AI_PROVIDER '${input.provider}'.`);
    if (!configuredModel)
        throw new Error('AI_MODEL is required so the gateway has an explicit default model.');
...
export const resolveGatewaySelection = (policy: GatewayModelPolicy, body: unknown): {
    provider: AIProviderId;
    model: string;
} | {
    error: string;
...
export const allowedModelCatalog = (policy: GatewayModelPolicy, discovered: AIModelOption[]): AIModelOption[] => {
    ...
    return Array.from(policy.allowedModels, id => discoveredById.get(id) || {
        id,
