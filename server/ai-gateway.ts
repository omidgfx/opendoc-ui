...
const validateMessages = (messages: unknown): messages is AIRequestMessage[] => {
    ...
    return messages.every(message => {
        ...
        if (!['system', 'user', 'assistant'].includes(item.role) || typeof item.content !== 'string' || item.content.length > maxMessageChars)
            return false;
...
app.post('/api/ai/chat', async (req, res) => {
    ...
    try {
        await streamAIResponse(baseSettings(selection.model, req.body?.temperature), messages, {
            ...
            onToken: token => { if (!res.writableEnded)
