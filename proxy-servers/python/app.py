...
def execute_proxied_request(raw_body: bytes, request_headers: Mapping[str, str], config: Config):
    ...
    if not config.proxy_enabled:
        raise DownloaderError("PROXY_DISABLED", "The request proxy is disabled on this service.", 403)
    target_url = (request_headers.get("x-opendoc-target-url") or "").strip()
    if not target_url:
        raise DownloaderError("MISSING_TARGET_URL", "Missing X-OpenDoc-Target-Url header.", 400)
    ...
    if not method.isalpha():
        raise DownloaderError("INVALID_TARGET_METHOD", "Target method is not valid.", 400)
    ...
    if raw_headers:
        try:
            ...
            raise DownloaderError("INVALID_TARGET_HEADERS", "X-OpenDoc-Target-Headers must be a JSON object.", 400) from error
        if not isinstance(parsed_headers, dict):
            raise DownloaderError("INVALID_TARGET_HEADERS", "X-OpenDoc-Target-Headers must be a JSON object.", 400)
    ...
    for redirect_count in range(config.max_redirects + 1):
        ...
        while True:
            ...
            if total > config.max_bytes:
                connection.close()
                raise DownloaderError("REMOTE_RESPONSE_TOO_LARGE", "Remote response exceeds OPENDOC_MAX_BYTES.", 502)
            chunks.append(chunk)
        connection.close()
        return {
            "status": status,
    ...
    class DownloaderHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            if not within_rate_limit(client, config.rate_limit):
                self.error_json(DownloaderError("RATE_LIMITED", "Proxy rate limit exceeded.", 429))
                return
            length = positive_int(self.headers.get("Content-Length"), 0)
            if length > config.max_bytes:
                self.error_json(DownloaderError("REQUEST_BODY_TOO_LARGE", "Proxied request body exceeds OPENDOC_MAX_BYTES.", 413))
                return
