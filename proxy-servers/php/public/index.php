...
use function OpenDoc\Downloader\executeProxiedRequest;
...
if ($path === '/proxy') {
    ...
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        sendError(new DownloaderException('METHOD_NOT_ALLOWED', 'Only POST and OPTIONS are allowed.', 405), $origin, $config);
    }
    $client = trim(explode(',', (string) ($_SERVER['HTTP_X_FORWARDED_FOR'] ?? $_SERVER['REMOTE_ADDR'] ?? ''))[0]);
    if (!withinRateLimit($client, $config['rateLimit'])) {
        sendError(new DownloaderException('RATE_LIMITED', 'Proxy rate limit exceeded.', 429), $origin, $config);
    }
    $rawBody = file_get_contents('php://input') ?: '';
    if (strlen($rawBody) > $config['maxBytes']) {
        sendError(new DownloaderException('REQUEST_BODY_TOO_LARGE', 'Proxied request body exceeds OPENDOC_MAX_BYTES.', 413), $origin, $config);
    }
