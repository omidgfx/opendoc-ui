...
public sealed class DownloaderService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!config.ProxyEnabled)
            throw new DownloaderException("PROXY_DISABLED", "The request proxy is disabled on this service.", 403);
        var target = incoming["X-OpenDoc-Target-Url"].FirstOrDefault()?.Trim() ?? "";
        if (target.Length == 0)
            throw new DownloaderException("MISSING_TARGET_URL", "Missing X-OpenDoc-Target-Url header.", 400);
        ...
        if (!method.All(char.IsLetter))
            throw new DownloaderException("INVALID_TARGET_METHOD", "Target method is not valid.", 400);
        ...
        if (rawHeaders.Length > 0)
        {
            ...
            {
                throw new DownloaderException("INVALID_TARGET_HEADERS", "X-OpenDoc-Target-Headers must be a JSON object.", 400, error);
            }
            if (parsed is null)
                throw new DownloaderException("INVALID_TARGET_HEADERS", "X-OpenDoc-Target-Headers must be a JSON object.", 400);
        ...
        for (var redirects = 0; redirects <= config.MaxRedirects; redirects++)
        {
            ...
            request.Headers.UserAgent.ParseAdd("OpenDoc-Request-Proxy/0.1");
            ...
            using (response)
            {
                ...
                while (true)
                {
                    ...
                    if (memory.Length + read > config.MaxBytes)
                        throw new DownloaderException("REMOTE_RESPONSE_TOO_LARGE", "Remote response exceeds OPENDOC_MAX_BYTES.", 502);
...
public static class EndpointExtensions
{
    public static IEndpointRouteBuilder MapOpenDocSpecDownloader(this IEndpointRouteBuilder endpoints, DownloaderConfig config)
    {
        ...
        endpoints.MapPost("/proxy", async (HttpContext context) =>
        {
            ...
            {
                return Error("DOWNLOADER_ERROR", "Proxied request failed.", 502);
            }
        });
