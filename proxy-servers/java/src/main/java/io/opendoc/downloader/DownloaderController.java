...
public final class DownloaderController {
    ...
    public ResponseEntity<Map<String, Object>> proxy(HttpServletRequest request) {
        ...
        if (!rateLimiter.allow(client, service.config().rateLimit()))
            throw new DownloaderException("RATE_LIMITED", "Proxy rate limit exceeded.", 429);
        byte[] rawBody;
        try {
            rawBody = request.getInputStream().readNBytes((int) service.config().maxBytes() + 1);
        } catch (java.io.IOException error) {
            throw new DownloaderException("DOWNLOADER_ERROR", "Proxied request body could not be read.", 400, error);
        }
        if (rawBody.length > service.config().maxBytes())
            throw new DownloaderException("REQUEST_BODY_TOO_LARGE", "Proxied request body exceeds OPENDOC_MAX_BYTES.", 413);
