...
import {applyThemeCssVariables, createThemeCssVariables} from '../themeCss';
...
export function useThemeController(selectedSpecKey: string) {
    ...
    // Restoration and persistence intentionally cross a render boundary so the
    // outgoing spec's theme can never be written under the incoming key.
    useEffect(() => {
