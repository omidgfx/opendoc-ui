...
import {VIEW_TAB_META, type TabItem, type ViewTabKind} from '../../components/endpoint/EndpointTabs';
                    ...
                    {tabs.map((tab, index) => {
                        ...
                        return (
                            <button key={tab.id} type="button" onClick={() => onSelect(tab.id)}
                                ...
                                {tab.isPreview && (
                                    <span className="shrink-0 text-[8px] font-black uppercase tracking-wider text-[var(--text-muted)]">Preview</span>
                                )}
                                {selected && <i className="ph ph-caret-right text-[14px] text-[var(--primary)] shrink-0"/>}
                            </button>
                        );
                    })}
                </div>
