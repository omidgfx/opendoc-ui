...
export function useLocalSpecifications({selectedSpecKey, onApply}: UseLocalSpecificationsOptions) {
    ...
    const storeAndApply = useCallback(
        (options: {
            ...
            if (diagnostics.length) registerSpecDiagnostics(document, diagnostics);
            try {
    ...
    const applyLocalBundle = useCallback(
        async (bundle: Record<string, string>, preferredFile?: File | null) => {
            const files = Object.entries(bundle).map(([name, raw]) => ({name, raw}));
            const processed = await processLocalOpenApiBundle(files);
            const rootFile =
                preferredFile && (preferredFile.webkitRelativePath || preferredFile.name) === processed.rootName
                    ? preferredFile
                    : null;
