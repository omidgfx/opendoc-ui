...
const topModalSurface = (): HTMLElement | null => {
    const surfaces = Array.from(document.querySelectorAll<HTMLElement>('.modal-surface')).filter(
...
export function useModalTransition(isOpen: boolean, onClose: () => void, durationMs = 180) {
    ...
    useEffect(() => {
        ...
        const remember = (target: EventTarget | null) => {
            if (!(target instanceof HTMLElement) || target.closest('.modal-surface, .modal-backdrop')) return;
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (previous?.isConnected) setTimeout(() => previous.focus({preventScroll: true}), 0);
        };
    }, [isOpen]);
