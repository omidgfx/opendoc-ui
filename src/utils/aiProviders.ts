...
const modelTier = (model: any): 'free' | 'premium' | 'local' => {
    ...
    if (model?.pricing && String(model.pricing.prompt) === '0' && String(model.pricing.completion) === '0') return 'free';
    return 'premium';
};
...
const consumeEventStream = async (
    ...
    const consumeLine = (line: string) => {
        ...
        try {
            ...
            // Providers occasionally emit non-JSON SSE comments; ignore only
...
const requestAnthropic = async (
    ...
    const response = await fetch(`${providerBaseUrl(settings)}/v1/messages`, {
        ...
        headers: {
            ...
            'x-api-key': settings.apiKey,
            'anthropic-version': '2023-06-01',
            // Anthropic requires this opt-in header for direct browser usage.
            'anthropic-dangerous-direct-browser-access': 'true',
        },
        body: JSON.stringify({
            model: settings.model,
...
const requestGateway = async (
    ...
    const response = await fetch(gatewayUrl(settings), {
        ...
        body: JSON.stringify({
            ...
            temperature: settings.temperature,
            stream: true,
            // The gateway owns credentials. The browser key is intentionally not forwarded.
        }),
    });
    await ensureOk(response);
    const contentType = response.headers.get('content-type') || '';
