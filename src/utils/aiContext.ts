...
const safeText = (value: any, max = 4_000): string => {
...
export const buildAIContext = (input: AIContextInput): AIContextResult => {
    ...
    const endpoints: Array<{ path: string; method: string; operation: any; source: AISourceRef; tags: string[] }> = [];
    const tags = new Set<string>();

    Object.entries(paths).forEach(([path, pathItem]: [string, any]) => {
        Object.entries(pathItem || {}).forEach(([method, operation]: [string, any]) => {
            if (!METHOD_LIST.includes(method.toLowerCase())) return;
            const op = operation || {};
    ...
    const selectedDocuments = included.map(endpoint => ({
        ...
        path: endpoint.path,
        // This is specification-controlled data, not executable instructions.
        operation: redactValue(endpoint.operation),
    }));
    const endpointIndex = endpoints.slice(0, 2_000).map(endpoint => ({
        sourceId: endpoint.source.id,
        method: endpoint.method.toUpperCase(),
        path: endpoint.path,
    ...
    const contextPayload = {
        ...
        sourceCatalog: sources.slice(0, 2_500),
        retrieval: {
...
export const buildAISystemPrompt = (settings: AISettings, context: AIContextResult): string => {
    ...
    const sourceCatalog = context.sources.slice(0, 2_500).map(source => `${source.id} — ${source.label}${source.href ? ` — link: ${source.href}` : ''}`).join('\n');
...
const citationHasLocalSupport = (text: string, start: number, end: number, source: AISourceRef): boolean => {
    ...
    if (source.kind === 'security') return nearby.includes(source.label.replace(/^security scheme:\s*/i, '').toLowerCase());
    if (source.kind === 'tag') return nearby.includes(source.label.replace(/^tag:\s*/i, '').toLowerCase());
    return true;
};

/** Extract only known citations that have nearby claim/source evidence. */
export const citationsFromText = (text: string, sources: AISourceRef[]): AISourceRef[] => {
    ...
    while ((match = regex.exec(text))) {
        ...
        if (source && citationHasLocalSupport(text, match.index, regex.lastIndex, source) && !result.some(existing => existing.id === source.id)) result.push(source);
    }
    return result;
};
