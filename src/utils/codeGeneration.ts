...
export type CodeLanguage = 'curl' | 'js-fetch' | 'js-axios' | 'angular' | 'laravel' | 'php' | 'python' | 'go' | 'csharp';
...
const placeholder = (name: string) => `YOUR_${String(name).replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toUpperCase() || 'VALUE'}`;
const SECRET_NAME = /(authorization|api[-_ ]?key|token|secret|password|passwd|cookie|credential|private[-_ ]?key|client[-_ ]?secret)/i;
const redactExampleSecrets = (value: unknown, key = '', seen = new WeakSet<object>()): unknown => {
    ...
    if (Array.isArray(value))
        return value.map(item => redactExampleSecrets(item, key, seen));
    return Object.fromEntries(Object.entries(value as Record<string, unknown>)
        .map(([childKey, child]) => [childKey, redactExampleSecrets(child, childKey, seen)]));
};
...
const placeholderAuth = (auth: ActiveAuth): ActiveAuth => {
    ...
    return {
        ...
        schemeValues: Object.fromEntries(Object.entries(normalized.schemeValues).map(([id, credential]) => [id, {
            ...credential,
            value: credential.type === 'bearer' || credential.type === 'oauth2' || credential.type === 'openIdConnect'
                ? 'YOUR_ACCESS_TOKEN'
                : credential.type === 'apiKey' || credential.type === 'cookie' ? placeholder(credential.name || id) : credential.value,
...
const bodyPreview = (spec: OpenApiSpec, operation: Operation): {body?: string; bodyType?: string} => {
    ...
    if (media.schema !== undefined) {
        ...
        if (generated.ok)
            return {body: JSON.stringify(redactExampleSecrets(generated.value), null, 2), bodyType};
    }
    ...
    getMergedParameters(pathItem, input.operation, input.spec).forEach((parameter: any) => {
        ...
        if (value === undefined) {
            if (schema?.type === 'array')
                value = [placeholder(`${parameter.name}_ITEM`)];
            else if (schema?.type === 'object')
                value = {[placeholder('KEY')]: placeholder('VALUE')};
            else
        ...
        if (value && typeof value === 'object' && !Array.isArray(value)) {
            Object.entries(value as Record<string, unknown>).forEach(([name, item]) => {
                const rendered = /file|upload|image|document/i.test(name)
                    ? `@/path/to/${name}`
                    : typeof item === 'object' ? `${JSON.stringify(item)};type=application/json` : String(item ?? '');
...
const fetchSnippet = (request: CodegenRequest): string => {
    const options: string[] = [`method: ${json(request.method)}`, `headers: ${JSON.stringify(request.headers, null, 2)}`];
    let prelude = '';
    if (request.bodyKind === 'multipart' && request.body) {
    ...
    if (request.cookies.length)
        options.push(`credentials: "include" // Browser sends only browser-managed cookies`);
...
const phpArray = (record: Record<string, string>) => `[${Object.entries(record).map(([key, value]) => `\n        ${json(key)} => ${json(value)},`).join('')}\n    ]`;
const phpSnippet = (request: CodegenRequest): string => {
    ...
    const multipartValue = multipart ? `[${Object.entries(fields).map(([name, value]) => /file|upload|image|document/i.test(name)
        ? `\n        ${json(name)} => new CURLFile('/path/to/${name}'),`
        : `\n        ${json(name)} => ${json(typeof value === 'object' ? JSON.stringify(value) : String(value ?? ''))},`).join('')}\n    ]` : null;
...
const laravelSnippet = (request: CodegenRequest): string => {
    ...
    const data = multipart ? Object.fromEntries(Object.entries(fields).filter(([name]) => !attachments.includes(name))) : {};
...
export const generateRequestSnippet = (language: CodeLanguage, request: CodegenRequest): string => {
    switch (language) {
        case 'curl': return curlSnippet(request);
        case 'js-fetch': return fetchSnippet(request);
        case 'js-axios': return axiosSnippet(request);
        case 'python': return pythonSnippet(request);
        case 'php': return phpSnippet(request);
        case 'go': return goSnippet(request);
        case 'csharp': return csharpSnippet(request);
        case 'angular': return angularSnippet(request);
        case 'laravel': return laravelSnippet(request);
    }
};
