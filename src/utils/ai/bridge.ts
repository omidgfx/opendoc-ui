...
export const createOpenDocUIActionId = (): string =>
...
const isParams = (value: unknown): value is Record<string, string | string[]> =>
    ...
    Object.values(value).every(
        item => typeof item === 'string' || (Array.isArray(item) && item.every(part => typeof part === 'string')),
    );
...
const normalizeEndpoint = (value: Record<string, any>): OpenDocUIAction | null => {
    if (
        ...
        !HTTP_METHOD_TOKEN.test(value.method)
    )
        return null;
...
const validateAction = (value: unknown): OpenDocUIAction | null => {
    ...
    if (value.action === 'open_schema' && typeof value.schema === 'string' && value.schema.trim())
        return {
            action: 'open_schema',
            schema: value.schema.trim(),
        };
    if (value.action === 'search_spec' && typeof value.query === 'string')
        return {
            action: 'search_spec',
            query: value.query.slice(0, 300),
        };
    if (value.action === 'select_server' && typeof value.url === 'string' && /^https?:\/\//i.test(value.url))
        return {
            action: 'select_server',
            url: value.url,
        };
...
export const parseOpenDocUIActions = (text: string): OpenDocUIAction[] => {
    ...
    const blockRegex =
        /<opendoc-ui-action>\s*([\s\S]*?)\s*<\/opendoc-ui-action>|```(?:opendoc-ui-action|json)\s*([\s\S]*?)```/gi;
...
export const stripOpenDocUIActionBlocks = (text: string): string =>
    text
...
export const actionLabel = (action: OpenDocUIAction): string => {
    ...
    if (action.action === 'set_runner_fields')
        return `Fill Runner fields for ${action.method.toUpperCase()} ${action.path}`;
...
const redactRunnerBody = (body: string): string =>
    body
        .replace(
...
export const formatOpenDocUIRunnerResult = (payload: OpenDocUIRunnerResult): string => {
    ...
    const status =
        result.status === 0 ? 'Network error' : result.status === null ? 'No response' : String(result.status);
    const details = [
        `### API Runner result`,
        `**${payload.method.toUpperCase()} ${payload.path}** — **${status}**${result.durationMs !== undefined ? ` in ${result.durationMs} ms` : ''}.`,
    ];
    if (result.requestUrl) details.push(`Request URL: ${result.requestUrl}`);
    const safeHeaders = Object.entries(result.headers || {}).filter(
        ([name]) => !/authorization|cookie|token|secret|password|api[-_ ]?key/i.test(name),
    );
    if (safeHeaders.length > 0)
        details.push(`Response headers: ${safeHeaders.map(([name, value]) => `${name}: ${value}`).join('; ')}`);
