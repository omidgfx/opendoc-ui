...
export const getProviderPreset = (id: AIProviderId): AIProviderPreset =>
    AI_PROVIDER_PRESETS.find(provider => provider.id === id) || AI_PROVIDER_PRESETS[0];
...
const gatewayUrl = (settings: AISettings): string => {
    ...
    if (!value) throw new Error('Configure an AI gateway URL or switch transport to Direct.');
    if (value.endsWith('/chat')) return value;
    if (value.endsWith('/api/ai')) return `${value}/chat`;
...
const modelListUrl = (settings: AISettings): string => {
    const base = providerBaseUrl(settings);
    if (settings.provider === 'ollama') return `${base.replace(/\/v1$/, '')}/api/tags`;
    if (settings.provider === 'gemini') return `${base}/v1beta/models?key=${encodeURIComponent(settings.apiKey)}`;
    return base.endsWith('/models') ? base : `${base}/models`;
};
const modelTier = (model: any): 'free' | 'premium' | 'local' => {
    if (typeof model?.id === 'string' && model.id.endsWith(':free')) return 'free';
...
export const fetchProviderModelCatalog = async (
    settings: AISettings,
    options: {
        signal?: AbortSignal;
    } = {},
    ...
    if (settings.transport === 'gateway') {
        ...
        if (rawGateway !== undefined) {
            if (
                !rawGateway ||
                typeof rawGateway !== 'object' ||
                !isProviderId(rawGateway.provider) ||
                typeof rawGateway.model !== 'string'
            ) {
            ...
            gateway = {
                ...
                models: Array.isArray(rawGateway.models)
                    ? rawGateway.models.filter((model: unknown): model is string => typeof model === 'string')
                    : undefined,
            };
        }
    ...
    const rawModels =
        settings.provider === 'ollama'
            ? Array.isArray(body.models)
                ? body.models.map((item: any) => ({id: item.name, name: item.name}))
                : []
    ...
    const models = rawModels
        ...
        .sort((a, b) =>
            a.tier === b.tier ? a.label.localeCompare(b.label) : a.tier === 'free' ? -1 : b.tier === 'free' ? 1 : 0,
        );
    return {models};
};
...
const errorFromPayload = (
    ...
    const message =
        typeof rawError?.message === 'string' && rawError.message.trim() ? rawError.message : fallback.message;
...
const errorFromResponse = async (response: Response): Promise<AIStreamError> => {
    ...
    try {
        ...
        return (
            errorFromPayload(parsed, {status: response.status, message: `${response.status} ${response.statusText}`}) ||
...
const contentFromPayload = (payload: any): string => {
    ...
    if (Array.isArray(openAIContent)) return openAIContent.map((part: any) => part?.text || '').join('');
    const anthropicContent = payload?.content;
    if (Array.isArray(anthropicContent)) return anthropicContent.map((part: any) => part?.text || '').join('');
    const geminiParts = payload?.candidates?.[0]?.content?.parts;
    if (Array.isArray(geminiParts)) return geminiParts.map((part: any) => part?.text || '').join('');
...
const deltaFromPayload = (payload: any): string => {
    ...
    if (Array.isArray(geminiDelta)) return geminiDelta.map((part: any) => part?.text || '').join('');
    return '';
};
...
const asOpenAIMessages = (messages: AIRequestMessage[]) =>
    messages.map(message => ({
        role: message.role,
        content: message.content,
    }));
const requestCompatible = async (
    settings: AISettings,
...
export const streamAIResponse = async (
    ...
    if (settings.provider === 'gemini') return requestGemini(settings, messages, options.signal, onToken);
