...
import {sessionStore, specStorage, uiStorage} from '../storage/index';
import {idbClearPrefix, idbDelete, idbGet, idbSet} from '../storage/indexedDb';
...
const normalizeSettings = (value: Partial<AISettings> | null | undefined): AISettings => {
    const provider = PROVIDERS.includes(value?.provider as AIProviderId)
    ...
    return {
        transport:
            value?.transport === 'gateway' || value?.transport === 'direct'
                ? value.transport
                : DEFAULT_AI_SETTINGS.transport,
        ...
        maxTokens:
            typeof value?.maxTokens === 'number' && Number.isFinite(value.maxTokens)
                ? Math.max(256, Math.min(16384, Math.floor(value.maxTokens)))
...
const isModelOption = (value: any): value is AIModelOption =>
    isRecord(value) &&
    typeof value.id === 'string' &&
    typeof value.label === 'string' &&
    (value.tier === 'free' || value.tier === 'premium' || value.tier === 'local');
const isProfile = (value: any): value is AIProfile =>
    isRecord(value) &&
    typeof value.id === 'string' &&
...
export const readAIProfiles = (): AIProfile[] => {
    const profiles = uiStorage.getJSON<AIProfile[]>(
        AI_PROFILES_KEY,
        [],
        value => Array.isArray(value) && value.length <= 30 && value.every(isProfile),
    );
...
export const writeAIProfiles = (profiles: AIProfile[]) => {
    ...
    Object.keys(secrets)
        .filter(key => key.startsWith('profile:') && !activeIds.has(key))
        .forEach(key => delete secrets[key]);
...
export const readAIModelCatalogs = (): Partial<Record<AIProviderId, AIModelOption[]>> => {
    const catalogs = uiStorage.getJSON<Partial<Record<AIProviderId, AIModelOption[]>>>(
        AI_MODEL_CATALOGS_KEY,
        {},
        value =>
            isRecord(value) &&
            Object.values(value).every(
                models => Array.isArray(models) && models.length <= 1000 && models.every(isModelOption),
            ),
    );
    return catalogs;
};
...
const gatewayCatalogKey = (gatewayUrl: string, provider: AIProviderId): string =>
    `${gatewayUrl.trim().replace(/\/+$/, '')}|${provider}`;
export const readAIGatewayModelCatalog = (gatewayUrl: string, provider: AIProviderId): AIModelOption[] => {
    const catalogs = uiStorage.getJSON<Record<string, AIModelOption[]>>(
        AI_GATEWAY_MODEL_CATALOGS_KEY,
        {},
        value =>
            isRecord(value) &&
            Object.values(value).every(
...
export const writeAIGatewayModelCatalog = (gatewayUrl: string, provider: AIProviderId, models: AIModelOption[]) => {
    const catalogs = uiStorage.getJSON<Record<string, AIModelOption[]>>(
        ...
        value =>
            isRecord(value) &&
            Object.values(value).every(
                items => Array.isArray(items) && items.length <= 1000 && items.every(isModelOption),
            ),
    );
...
const isSource = (value: any): value is AISourceRef =>
    isRecord(value) &&
    typeof value.id === 'string' &&
    typeof value.label === 'string' &&
    ['endpoint', 'schema', 'tag', 'security', 'server', 'spec'].includes(value.kind);
const isMessage = (value: any): value is AIChatMessage =>
    isRecord(value) &&
    typeof value.id === 'string' &&
...
const isConversation = (value: any): value is AIConversation =>
    isRecord(value) &&
    typeof value.id === 'string' &&
...
export const trimAIConversation = (conversation: AIConversation): AIConversation => {
    ...
    if (messages.length <= MAX_CONVERSATION_MESSAGES) return {...conversation, messages};
...
export const readAIConversations = (specKey: string): AIConversation[] => {
    if (!specKey) return [];
    const conversations = specStorage.getJSON<AIConversation[]>(
        specKey,
        AI_CONVERSATIONS_KEY,
        [],
        value => Array.isArray(value) && value.length <= 30 && value.every(isConversation),
    );
...
const normalizedConversations = (specKey: string, conversations: AIConversation[]): AIConversation[] =>
    conversations
        .filter(conversation => conversation.specKey === specKey)
        .map(trimAIConversation)
        .slice(0, 30)
...
export const writeAIConversations = (specKey: string, conversations: AIConversation[]) => {
    ...
    if (!result) console.warn('OpenDoc UI could not persist the conversation mirror; IndexedDB may still contain it.');
    return result;
};
