...
const selectedSchemeIds = (auth: ActiveAuth, spec: OpenApiSpec | null): string[] => {
    ...
    if (normalized.selectedSchemes.length > 0) return normalized.selectedSchemes;
    if (normalized.activeScheme !== 'none' && spec?.components?.securitySchemes?.[normalized.activeScheme]) return [normalized.activeScheme];
...
const credentialFor = (auth: ActiveAuth, id: string, scheme: any): AuthCredential => {
    ...
    if (type === 'bearer' || type === 'oauth2' || type === 'openIdConnect') return {
        schemeId: id,
        type,
...
/** Apply every selected security scheme to a request without collapsing IDs. */
export const applyAuthToRequest = (
    spec: OpenApiSpec | null,
    auth: ActiveAuth,
    request: { headers?: Record<string, string>; query?: SerializedPair[]; cookies?: SerializedPair[] },
    ...
    ids.forEach(id => {
        ...
        if (scheme.type === 'apiKey' || credential.type === 'apiKey') {
            ...
            if (!value && location !== 'cookie') warnings.push(`No value is configured for API-key scheme '${id}'.`);
            if (location === 'query' && value) query.push({name, value, allowReserved: false});
            else if (location === 'header' && value) headers[name] = value;
        ...
        if (scheme.type === 'http' && schemeName === 'basic' || credential.type === 'basic') {
            if (credential.username) headers.Authorization = `Basic ${basicEncode(credential.username, credential.password || '')}`;
            else warnings.push(`No username is configured for HTTP basic scheme '${id}'.`);
            return;
        }
        if (scheme.type === 'http' && schemeName === 'bearer' || ['bearer', 'oauth2', 'openIdConnect'].includes(credential.type) || ['oauth2', 'openIdConnect'].includes(scheme.type)) {
            ...
            else warnings.push(`No access token is configured for scheme '${id}'. OAuth authorization-code/PKCE is not performed in the browser runner.`);
            return;
        }
        if (scheme.type === 'apiKey' && scheme.in === 'cookie') credentials = 'include';
    });

    // Cookie parameters and cookie API keys are intentionally not copied into a
    // forbidden browser Cookie header; the caller can show the warning above or
    // hand `cookies` to a trusted backend executor.
