...
const clone = <T, >(value: T): T => {
    ...
    try {
        ...
        // YAML/JSON input should be cloneable; retain the portable fallback for
        // unusual host objects supplied by an embedding application.
    }
...
const normalizeParameter = (param: any): any => {
    ...
    // Preserve Swagger 2 collectionFormat semantics in the OpenAPI 3 runner.
    if (next.collectionFormat) {
        const collectionStyle: Record<string, { style: string; explode?: boolean }> = {
...
const normalizeOpenApiLike = (input: any): OpenApiSpec => {
    // rewriteRefsDeep creates a fresh tree, so avoid the previous clone-then-
    // traverse double copy for large documents.
...
export const normalizeOpenApiSpec = (input: any): OpenApiSpec => {
    assertValidOpenApiDocument(input);
