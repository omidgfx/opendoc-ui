...
export const getPathItemOperations = (pathItem: PathItem | any): PathOperationEntry[] => {
    ...
    OAS_FIXED_HTTP_METHODS.forEach(method => {
        ...
        if (operation && typeof operation === 'object') entries.push({method, operation, source: 'fixed'});
    });
    if (pathItem.additionalOperations && typeof pathItem.additionalOperations === 'object') {
        Object.entries(pathItem.additionalOperations).forEach(([method, operation]) => {
            ...
            if (entries.some(entry => entry.method.toLowerCase() === normalized)) return;
...
export const getOperation = (spec: OpenApiSpec | null | undefined, path: string, method: string): Operation | null => {
...
export const getDocumentOperations = (
    spec: OpenApiSpec | null | undefined,
): Array<PathOperationEntry & {path: string}> => {
    if (!spec?.paths) return [];
    return Object.entries(spec.paths).flatMap(([path, pathItem]) =>
        getPathItemOperations(pathItem).map(entry => ({...entry, path})),
    );
};
