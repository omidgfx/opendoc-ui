...
const normalizeSettings = (value: Partial<AISettings> | null | undefined): AISettings => {
    ...
    return {
        ...
        temperature,
        maxTokens: typeof value?.maxTokens === 'number' && Number.isFinite(value.maxTokens) ? Math.max(256, Math.min(16_384, Math.floor(value.maxTokens))) : DEFAULT_AI_SETTINGS.maxTokens,
...
type SessionSecrets = Record<string, { apiKey?: string; gatewayToken?: string }>;
...
export const readAIProfiles = (): AIProfile[] => {
    const profiles = uiStorage.getJSON<AIProfile[]>(AI_PROFILES_KEY, [], value =>
        Array.isArray(value) && value.length <= 30 && value.every(isProfile),
    );
...
export const readAIModelCatalogs = (): Partial<Record<AIProviderId, AIModelOption[]>> => {
    const catalogs = uiStorage.getJSON<Partial<Record<AIProviderId, AIModelOption[]>>>(AI_MODEL_CATALOGS_KEY, {}, value =>
...
const isConversation = (value: any): value is AIConversation =>
    ...
    // Accept older oversized conversations so they can be repaired and
    // truncated safely instead of being deleted as corrupt data.
...
/** Keep a conversation usable at the storage boundary. The marker preserves
 * the fact that history exists without deleting the whole conversation. */
...
export const readAIConversations = (specKey: string): AIConversation[] => {
    if (!specKey) return [];
    const conversations = specStorage.getJSON<AIConversation[]>(specKey, AI_CONVERSATIONS_KEY, [], value =>
        Array.isArray(value) && value.length <= 30 && value.every(isConversation),
    );
...
export const writeAIConversations = (specKey: string, conversations: AIConversation[]) => {
    ...
    // IndexedDB is the primary large-data store. Keep a bounded synchronous
    // mirror through the IndexedDB-first storage facade for fallback browsers.
    if (next.length === 0) {
