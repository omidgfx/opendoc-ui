...
export const sanitizeZipEntryName = (input: string): string => {
    const normalized = String(input || 'file')
        .replace(/\\/g, '/')
        .replace(/^\/+/, '');
...
export function createZipBlob(files: ZipFileEntry[]): Blob {
    ...
    const prepared = files.map(f => {
        ...
        let name = sanitized;
        let suffix = 2;
        while (usedNames.has(name.toLowerCase())) name = `${stem}_${suffix++}${extension}`;
    ...
    for (const p of prepared) {
        totalLocalSize += 30 + p.nameBytes.length + p.contentBytes.length;
    ...
    for (const rec of centralRecords) {
        ...
        offset += 4;
        writeU32(view, offset, rec.offset);
        offset += 4;
        uint8.set(p.nameBytes, offset);
        offset += p.nameBytes.length;
    }
    ...
    writeU16(view, offset, 0);
    offset += 2;
    writeU16(view, offset, centralRecords.length);
    offset += 2;
    writeU16(view, offset, centralRecords.length);
    offset += 2;
