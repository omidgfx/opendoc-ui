...
const setHeader = (
    ...
    if (existing && headers[existing] !== value) {
        diagnostics.push(diagnostic(
            'RUN_HEADER_VALUE_REPLACED',
            `${source} replaced the existing '${existing}' header value.`,
            {severity: 'info'},
        ));
...
const valueForParameter = (input: CompileRequestInput, parameter: any): unknown => {
    ...
    if (parameter.in === 'header') {
        ...
        if (headerName)
            return input.headers?.[headerName];
    }
    return undefined;
};
...
const collectAcceptTypes = (operation: Operation): string[] => {
    ...
    Object.values(operation.responses || {}).forEach((response: any) => {
        Object.keys(response?.content || {}).forEach(mediaType => {
            if (!result.some(value => value.toLowerCase() === mediaType.toLowerCase()))
...
const createBodyIntent = (
    input: CompileRequestInput,
    diagnostics: Diagnostic[],
): RequestBodyIntent => {
    const requestBody = resolveRequestBody(input.operation.requestBody, input.spec);
    const content = requestBody?.content || {};
    const mediaType = input.bodyType || Object.keys(content)[0] || (input.selectedFile ? 'application/octet-stream' : 'application/json');
...
const unresolvedExternalRefDiagnostics = (value: unknown, seen = new Set<object>()): Diagnostic[] => {
    ...
    if (Array.isArray(value))
        return value.flatMap(item => unresolvedExternalRefDiagnostics(item, seen));
...
export const compileRequestIntent = (input: CompileRequestInput): RequestIntent => {
    ...
    mergedParameters.forEach((parameter: any) => {
        ...
        try {
            ...
            Object.entries(serialized.headers).forEach(([name, headerValue]) => setHeader(
                parameterHeaders,
                name,
                headerValue,
                diagnostics,
    ...
    setHeader(headers, 'Accept', acceptTypes.length > 0 ? acceptTypes.join(', ') : '*/*', diagnostics, 'OpenAPI response content');
    Object.entries(parameterHeaders).forEach(([name, value]) => setHeader(headers, name, value, diagnostics, 'OpenAPI parameter'));
    Object.entries(input.headers || {}).forEach(([name, value]) => setHeader(headers, name, String(value), diagnostics, 'Explicit Runner header'));
...
const firstContentType = (encoding: any): string | undefined => typeof encoding?.contentType === 'string'
...
const materializeMultipart = (body: RequestBodyIntent): FormData => {
    ...
    Object.entries(files).forEach(([stateKey, file]) => {
        if (!file || consumed.has(stateKey))
            return;
        const fieldName = stateKey.split('.').pop() || stateKey;
        const contentType = firstContentType(body.encoding?.[fieldName]);
        if (contentType && typeof Blob !== 'undefined') {
            const wrapped = new Blob([file], {type: contentType});
            const filename = 'name' in file && typeof (file as File).name === 'string' ? (file as File).name : fieldName;
...
export const compileBrowserRequest = (input: CompileRequestInput): RequestPlan => materializeBrowserRequest(
    compileRequestIntent(input),
);
