...
export const isValidTabViewMode = (value: unknown): value is StoredTabViewMode => VALID_TAB_VIEW_MODES.includes(value as StoredTabViewMode);
const isValidTabFilters = (filters: any): boolean => !!filters
...
export const isValidTabPersistence = (value: any): boolean => {
    ...
    if (value.viewModes !== undefined) {
        if (!value.viewModes || typeof value.viewModes !== 'object' || Array.isArray(value.viewModes))
            return false;
        if (!Object.values(value.viewModes).every(isValidTabViewMode))
            return false;
    }
    return true;
};
export const hasExplicitSpecRoute = (route: ParsedRoute, hash: string): boolean => !!(route.endpoint || route.legacyOperationId || route.showSchemaExplorer || route.showAbout
