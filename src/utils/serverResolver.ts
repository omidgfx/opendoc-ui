...
const resolveRelativeUrl = (expanded: string, sourceUri: string | undefined): {url: string; diagnostic?: Diagnostic} => {
    if (!expanded)
        return {
            url: expanded,
            diagnostic: diagnostic('OAS_SERVER_URL_EMPTY', 'The selected server URL is empty. Browser fetch will report the resulting error.'),
        };
    ...
    try {
        return {
            ...
            diagnostic: sourceUri ? undefined : diagnostic(
...
export const resolveEffectiveServer = (input: ResolveServerInput): ResolvedServer => {
    ...
    const customRootSelection = !matched && source === 'root' && selected
    ...
    return {
        ...
        diagnostics: [
            ...candidate.expansion.diagnostics,
            ...(relative.diagnostic ? [relative.diagnostic] : []),
        ],
    };
};

export const effectiveServerDefinitions = (input: Omit<ResolveServerInput, 'selectedServer'>): Array<{
    source: ServerSource;
    definition: ServerDefinition;
    expandedUrl: string;
}> => {
