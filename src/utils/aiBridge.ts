...
const normalizeEndpoint = (value: Record<string, any>): OpenDocUIAction | null => {
    ...
    if (value.action === 'set_runner_fields' || value.action === 'run_api') {
        if (value.params !== undefined && !isParams(value.params))
            return null;
        if (value.headers !== undefined && !isHeaders(value.headers))
            return null;
        if (value.body !== undefined && typeof value.body !== 'string')
            return null;
        if (value.bodyType !== undefined && typeof value.bodyType !== 'string')
            return null;
...
export const actionLabel = (action: OpenDocUIAction): string => {
    if (action.action === 'open_endpoint')
        return `Open ${action.method.toUpperCase()} ${action.path}`;
    if (action.action === 'open_runner')
        return `Open Runner for ${action.method.toUpperCase()} ${action.path}`;
    ...
    if (action.action === 'run_api')
        return `Fill and run ${action.method.toUpperCase()} ${action.path}`;
    if (action.action === 'open_schema')
        return `Open schema ${action.schema}`;
...
export const formatOpenDocUIRunnerResult = (payload: OpenDocUIRunnerResult): string => {
    ...
    if (result.requestUrl)
        details.push(`Request URL: ${result.requestUrl}`);
    ...
    if (result.body)
        details.push('', '```' + (result.isJson ? 'json' : 'text'), redactRunnerBody(result.body), '```');
    if (result.truncated)
        details.push('', '_The response preview was truncated by the Runner limit._');
    if (result.isBinary)
        details.push('', '_The response is binary and was not rendered as text._');
    return details.join('\n');
};
