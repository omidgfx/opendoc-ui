...
export default function SchemaViewerHeader({
    ...
    return (
        <header className="px-4 sm:px-6 py-2.5 sm:py-4 flex flex-col gap-2 sm:gap-3 border-b shrink-0 border-[var(--border)] bg-[var(--background)] modal-header-mobile-pad">
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    >
                        <i className="ph ph-diamonds-four text-[20px] sm:text-[24px]" />
                    </span>
                    <div className="min-w-0">
                        <h3 className="font-semibold text-sm sm:text-base text-[var(--text-heading)] truncate">
                            {active.schemaName}
                        </h3>
                        <p className="text-[10px] sm:text-xs text-[var(--text-muted)] truncate">Schema Explorer</p>
                    </div>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                    <Tip content="Share this schema link">
                        <button
                            onClick={() => onShare(active.schemaName)}
                        ...
                        >
                            <i className="ph ph-share-network text-[14px]" />
                        </button>
                    </Tip>
                    <Tip content="Export this schema as TypeScript model">
                        <button
                            onClick={() =>
                                schemas && generateSingleSchemaFile(active.schemaName, active.schema, schemas, specKey)
                            }
                        ...
                        >
                            <i className="ph ph-download-simple text-[12px]" />
            ...
            <div
                ...
                onWheel={wheel}
            >
                <span className="text-[var(--text-muted)] font-semibold flex items-center shrink-0">Path:</span>
                {stack.map((item, index) => {
                    const last = index === stack.length - 1;
                    return (
                        <div key={`${item.schemaName}-${index}`} className="flex items-center gap-1.5 shrink-0">
                            {index > 0 && <i className="ph ph-caret-right text-[9px] text-[var(--text-muted)]" />}
                            <button
                                disabled={last}
                                onClick={() => {
                                    for (let i = stack.length - 1; i > index; i--) onPop();
                                }}
                                className={`px-2 py-0.5 rounded text-[11px] font-semibold truncate max-w-[140px] ${last ? 'bg-[var(--primary)] text-[var(--primary-contrast)] pointer-events-none' : 'bg-[var(--text-muted)]/10 hover:bg-[var(--text-muted)]/20 text-[var(--primary)] cursor-pointer'}`}
                            >
                                {item.schemaName}
                            </button>
                        </div>
            ...
            <div className="text-[10px] text-[var(--text-muted)] flex items-center gap-1.5">
                ...
                <span>
                    Press{' '}
                    <kbd className="px-1 py-0.5 rounded border text-[9px] bg-[var(--surface-hover)] border-[var(--border)]">
                        ESC
                    </kbd>{' '}
                    to {stack.length > 1 ? 'go back' : 'close'}
                </span>
            </div>
        </header>
    );
