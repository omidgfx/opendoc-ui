...
export default function SchemaExampleModal({visible, backdropClassName, value, onClose}: SchemaExampleModalProps) {
    if (!visible || !value)
        return null;
    return <div className={`${backdropClassName} fixed inset-0 z-[3000] backdrop-blur-[2px]`}
        ...
        <div
            ...
            <header
                ...
                <span className="font-bold text-sm tracking-wide text-[var(--text-heading)]"><i
                    className={value.isJson ? 'ph ph-eye mr-1.5 text-[var(--primary)]' : 'ph ph-info mr-1.5 text-[var(--primary)]'}/>{value.title}
                </span>
            ...
            <div
                className="modal-scroll-region p-4 sm:p-6 overflow-y-auto space-y-4 text-xs leading-relaxed scrollbar-thin text-[var(--text)]">{value.isJson ?
                <CodeViewer code={value.content} language="json" maxHeight="none"/> :
                <div className="whitespace-pre-wrap opacity-95">{value.content}</div>}</div>
            <footer className="px-5 py-3 border-t text-right border-[var(--border)] bg-[var(--background)]">
                <button onClick={onClose}
                        className="px-4 py-1.5 text-[var(--primary-contrast)] font-semibold text-xs rounded-lg cursor-pointer bg-[var(--primary)]">{value.isJson ? 'Close Example' : 'Close Help'}</button>
            </footer>
        </div>
    </div>;
}
