...
export default function ShareModal({ isOpen, onClose, url, title, description }: ShareModalProps) {
  ...
  const shareOptions = [
    { name: 'WhatsApp', icon: 'ph-fill ph-whatsapp-logo', color: '#25D366', url: `https://wa.me/?text=${encodeURIComponent(`${shareText} ${originUrl}`)}` },
    { name: 'Telegram', icon: 'ph-fill ph-telegram-logo', color: '#0088cc', url: `https://t.me/share/url?url=${encodedUrl}&text=${encodedText}` },
    { name: 'Email',    icon: 'ph-fill ph-envelope',      color: '#EA4335', url: `mailto:?subject=${encodedText}&body=${encodeURIComponent(`${shareDesc}\n\n${originUrl}`)}` },
    { name: 'X',        icon: 'ph-fill ph-x-logo',        color: '#000000', url: `https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedText}` },
    { name: 'Facebook', icon: 'ph-fill ph-facebook-logo', color: '#1877F2', url: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}` },
    { name: 'LinkedIn', icon: 'ph-fill ph-linkedin-logo', color: '#0A66C2', url: `https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}` },
    { name: 'Reddit',   icon: 'ph-fill ph-reddit-logo',   color: '#FF4500', url: `https://www.reddit.com/submit?url=${encodedUrl}&title=${encodedText}` },
    { name: 'Pinterest',icon: 'ph-fill ph-pinterest-logo',color: '#E60023', url: `https://pinterest.com/pin/create/button/?url=${encodedUrl}&description=${encodedDesc}` },
  ];
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg rounded-2xl border shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 bg-[var(--surface)] border-[var(--border)] flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto scrollbar-thin">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-wider text-[var(--text-muted)]">Share via</label>
            <div className="grid grid-cols-4 gap-2 sm:gap-2.5">
              {shareOptions.map((opt) => (
                <Tip key={opt.name} content={`Share on ${opt.name}`}>
                  ...
                  >
                    <span className="size-8 sm:size-9 rounded-full flex items-center justify-center text-white shadow-sm group-hover:scale-105 transition-transform"
