...
export default function ApiSpecificationSelectorModal({
    ...
    if (!shouldRender)
        return null;
    return (<div className={`${backdropClassName} fixed inset-0 z-[2500] bg-black/60 backdrop-blur-[2px]`}
                 onMouseDown={(event) => {
                     if (event.target === event.currentTarget)
                         requestClose();
                 }}>
        <section role="dialog" aria-modal="true" aria-labelledby="api-specification-selector-title"
            ...
            {isLocalMode ? (<div className="modal-scroll-region min-h-0 flex-1 overflow-y-auto p-4 scrollbar-thin">
                ...
                {canOpenLocal && (<button type="button" onClick={onOpenLocalFile}
                ...
                <div className="mt-5 mb-2 flex items-center justify-between">
                    ...
                    {localHistory.length > 0 && (<button type="button" onClick={onClearHistory}
                ...
                {localHistory.length === 0 ? (
                    ...
                    {localHistory.map((entry) => {
                        ...
                        return (<div key={entry.key}
                                     className={clsx('flex items-center gap-3 rounded-2xl border p-3 transition-all', selected
                            ...
                            <Tip content="Reset this specification's saved configuration">
                                <button type="button" onClick={() => setConfirmAction({kind: 'spec', key: entry.key})}
                            ...
                            <button type="button" onClick={() => onRemoveHistoryEntry(entry.key)}
                                    className="size-7 shrink-0 rounded-lg flex items-center justify-center text-[var(--text-muted)] transition-colors hover:text-[var(--method-delete)] hover:bg-[var(--method-delete)]/10 cursor-pointer"
                        ...
                        {entries.map(([key, item]) => {
                            ...
                            return (<div role="button" tabIndex={0} key={key} onClick={() => {
                                ...
                                <div className="flex items-start gap-4">
                                    <span
                                        className={clsx('flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border text-[17px]', selected
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        {summary && (<>
                                            ...
                                            <div className="mt-3 flex flex-wrap gap-2">
                                                ...
                                                ].map(([label, value, icon]) => (<span key={String(label)}
                                            ...
                                            {summary.methods.length > 0 && (
                                                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                                                    ...
                                                    {summary.methods.map((method) => (<span key={method}
        ...
        {confirmTransition.shouldRender && confirmAction && (<div
            ...
            <section role="dialog" aria-modal="true" aria-labelledby="reset-config-confirm-title"
                ...
                <header
                    className="flex items-center gap-3 border-b border-[var(--border)] bg-[var(--background)] px-4 py-3">
                    <span
                        className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[var(--method-delete)]/10 text-[var(--method-delete)]"><i
                        className="ph ph-arrow-counter-clockwise text-[18px]"/></span>
                    <h3 id="reset-config-confirm-title"
                        className="text-sm font-extrabold text-[var(--text-heading)]">{confirmAction.kind === 'all' ? 'Reset all configurations?' : 'Reset this specification?'}</h3>
                </header>
                <div className="px-4 py-4"><p
                    className="text-[11px] leading-relaxed text-[var(--text-muted)]">{confirmAction.kind === 'all'
                    ? 'This clears saved general UI settings and all per-spec configurations, then reloads the application.'
                    : 'This clears the saved settings, tabs, inputs, theme, and navigation preferences for this specification, then reloads it.'}</p>
                </div>
                <footer
                    className="flex justify-end gap-2 border-t border-[var(--border)] bg-[var(--background)] px-4 py-3">
                    <button type="button" disabled={isConfirming} onClick={confirmTransition.requestClose}
                            className="rounded-xl border border-[var(--border)] px-4 py-2 text-xs font-bold text-[var(--text-heading)] hover:bg-[var(--surface-hover)] disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed">Cancel
                    </button>
