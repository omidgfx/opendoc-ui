...
function ThemePreviewCard({ theme, selected, resolvedThemeMode, onSelect }: { theme: AppTheme; selected: boolean; resolvedThemeMode: 'light' | 'dark'; onSelect: () => void }) {
...
function DetailedThemeView({ selectedTheme, selectedThemeName, currentThemeMode, resolvedThemeMode, onSelectTheme, onClose }: {
            ...
            <main className="min-w-0 flex-1 overflow-y-auto p-3 sm:p-4 md:p-5 scrollbar-thin">
                <div className="mb-4 flex items-center justify-between gap-3">
                    ...
                    <span className="inline-flex items-center gap-1.5 rounded-full border px-2 sm:px-3 py-1 sm:py-1.5 text-[9px] sm:text-[10px] font-bold shrink-0"
                        style={{ color: selectedTheme[resolvedThemeMode].primary, borderColor: alpha(selectedTheme[resolvedThemeMode].primary, '4d'), backgroundColor: alpha(selectedTheme[resolvedThemeMode].primary, '12') }}>
                        <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: selectedTheme[resolvedThemeMode].primary }} />
                        Active in {currentThemeMode === 'system' ? `system (${resolvedThemeMode})` : currentThemeMode} mode
                    </span>
                </div>
                ...
                <header className="flex shrink-0 items-center justify-between gap-2 border-b px-3 py-2.5 sm:px-4 sm:py-3 md:px-5 modal-header-mobile-pad bg-[var(--background)] border-[var(--border)]">
                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                        <span className="flex h-8 w-8 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-xl border text-[16px] sm:text-[18px]"
                            style={{ color: selectedTheme[resolvedThemeMode].primary, backgroundColor: alpha(selectedTheme[resolvedThemeMode].primary, '14'), borderColor: alpha(selectedTheme[resolvedThemeMode].primary, '3d') }}>
                ...
                <div className="min-h-0 flex-1">
                    {view === 'gallery' ? (
                        ...
                        <DetailedThemeView selectedTheme={selectedTheme} selectedThemeName={selectedThemeName}
                            currentThemeMode={currentThemeMode} resolvedThemeMode={resolvedThemeMode} onSelectTheme={onSelectTheme} onClose={onClose} />
                    )}
                </div>
