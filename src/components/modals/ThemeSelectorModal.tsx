...
export default function ThemeSelectorModal({ isOpen, selectedThemeName, currentThemeMode, resolvedThemeMode, onSelectTheme, onClose, onToggleThemeMode, onSetThemeMode }: ThemeSelectorModalProps) {
    ...
    if (!shouldRender)
        return null;
    return (<div className={`${backdropClassName} fixed inset-0 z-[2500]`} style={{ backgroundColor: 'rgba(7, 10, 18, .64)', backdropFilter: 'blur(6px)' }} onMouseDown={(event) => {
            if (event.target === event.currentTarget)
                requestClose();
        }}>

            <section role="dialog" aria-modal="true" aria-labelledby="theme-selector-title" className="modal-surface modal-surface-stable flex h-[92vh] sm:h-[86vh] w-full max-w-7xl flex-col overflow-hidden rounded-2xl border shadow-2xl bg-[var(--surface)] border-[var(--border)] text-[var(--text)]">
                ...
                <header className="flex shrink-0 items-center justify-between gap-2 border-b px-3 py-2.5 sm:px-4 sm:py-3 md:px-5 modal-header-mobile-pad bg-[var(--background)] border-[var(--border)]">
                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                        {view === 'detail' && (<button type="button" onClick={() => setFocusSidebarOpen(true)} className="flex size-8 shrink-0 items-center justify-center rounded-xl border border-[var(--border)] text-[var(--text-heading)] hover:bg-[var(--surface-hover)] md:hidden" aria-label="Open theme list">
                                <i className="ph ph-list text-[15px]"/>
                            </button>)}
                        <span className="flex h-8 w-8 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-xl border text-[16px] sm:text-[18px]" style={{
            ...
            borderColor: alpha(selectedTheme[resolvedThemeMode].primary, '3d')
                        ...
                        <div className="min-w-0">
                            <h2 id="theme-selector-title" className="truncate text-sm font-extrabold tracking-tight text-[var(--text-heading)]">Choose
                                your theme</h2>
            ...
            string,
            string
        ][]).map(([mode, icon, tip]) => (<Tip key={mode} content={tip}>
                                    <button type="button" onClick={() => onSetThemeMode(mode)} aria-pressed={currentThemeMode === mode} className={clsx('size-8 sm:size-9 rounded-lg flex items-center justify-center transition-all cursor-pointer', currentThemeMode === mode
                ...
                : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-heading)]')}>
                        ...
                        <Tip content="Close">
                            <button type="button" onClick={requestClose} autoFocus className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl border text-[14px] transition-all cursor-pointer hover:bg-[var(--surface-hover)] border-[var(--border)] text-[var(--text-muted)]" aria-label="Close theme selector">
                                <i className="ph ph-x"/>
                            </button>
                        </Tip>
                    </div>
                ...
                <div className="min-h-0 flex-1">
                    {view === 'gallery' ? (<div className="modal-scroll-region h-full overflow-y-auto p-3 sm:p-4 md:p-5 scrollbar-thin">
                            ...
                            <div className="grid grid-cols-2 gap-2.5 sm:gap-3 md:grid-cols-2 lg:grid-cols-4">
                                {THEME_LIST.map((theme) => <ThemePreviewCard key={theme.name} theme={theme} selected={theme.name === selectedThemeName} resolvedThemeMode={resolvedThemeMode} onSelect={() => onSelectTheme(theme.name)}/>)}
                            </div>
                        </div>) : (<DetailedThemeView selectedTheme={selectedTheme} selectedThemeName={selectedThemeName} currentThemeMode={currentThemeMode} resolvedThemeMode={resolvedThemeMode} onSelectTheme={onSelectTheme} onClose={requestClose} mobileSidebarOpen={focusSidebarOpen} onCloseMobileSidebar={() => setFocusSidebarOpen(false)}/>)}
                </div>
                ...
                <footer className="flex shrink-0 items-center justify-between gap-4 border-t px-3 py-2.5 sm:px-4 sm:py-3 md:px-5 modal-header-mobile-pad bg-[var(--background)] border-[var(--border)]">
                    <div className="flex min-w-0 items-center gap-2 text-[10px] text-[var(--text-muted)]">
                        <i className="ph ph-check-circle text-[14px]" style={{ color: selectedTheme[resolvedThemeMode].primary }}/>
                        <span className="truncate"><strong>{selectedTheme.name}</strong> is selected</span>
                    </div>
                    <button type="button" onClick={requestClose} className="rounded-xl px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-bold text-[var(--primary-contrast)] shadow-sm transition-all cursor-pointer hover:opacity-90 active:scale-[0.98]" style={{ backgroundColor: selectedTheme[resolvedThemeMode].primary }}>
                        Done
                    </button>
                </footer>
            </section>
