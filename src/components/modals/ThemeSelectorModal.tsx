...
export default function ThemeSelectorModal({
    ...
    return (
        <div
            className={`${backdropClassName} fixed inset-0 z-[2500]`}
            ...
            >
                <header className="flex shrink-0 items-center justify-between gap-2 border-b px-3 py-2.5 sm:px-4 sm:py-3 md:px-5 modal-header-mobile-pad bg-[var(--background)] border-[var(--border)]">
                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                        {view === 'detail' && (
                            <button
                                type="button"
                                onClick={() => setFocusSidebarOpen(true)}
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="mt-0.5 truncate text-[10px] text-[var(--text-muted)] hidden sm:block">
                    ...
                    <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
                        <Tip content={`Switch to ${view === 'gallery' ? 'focus' : 'gallery'} view`}>
                            ...
                            >
                                <i
                                    className={`ph ${view === 'gallery' ? 'ph-sidebar' : 'ph-squares-four'} text-[14px]`}
                                />
                            ...
                            {(
                                ...
                                <Tip key={mode} content={tip}>
                                    <button
                                        ...
                                        className={clsx(
                                            'size-8 sm:size-9 rounded-lg flex items-center justify-center transition-all cursor-pointer',
                    ...
                    {view === 'gallery' ? (
                        <div className="modal-scroll-region h-full overflow-y-auto p-3 sm:p-4 md:p-5 scrollbar-thin">
                            <div className="mb-4 flex items-end justify-between gap-3">
                                ...
                                <span className="rounded-full border px-2.5 py-1 text-[9px] font-bold border-[var(--border)] text-[var(--text-muted)] shrink-0">
                ...
                <footer className="flex shrink-0 items-center justify-between gap-4 border-t px-3 py-2.5 sm:px-4 sm:py-3 md:px-5 modal-header-mobile-pad bg-[var(--background)] border-[var(--border)]">
                    <div className="flex min-w-0 items-center gap-2 text-[10px] text-[var(--text-muted)]">
                        <i
                            className="ph ph-check-circle text-[14px]"
                            style={{color: selectedTheme[resolvedThemeMode].primary}}
                        />
                        <span className="truncate">
                            <strong>{selectedTheme.name}</strong> is selected
                        </span>
                    </div>
                    <button
                        type="button"
