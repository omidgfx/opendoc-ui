...
export default function ThemeSelectorModal({
    ...
    if (!shouldRender)
        return null;
    return (<div className={`${backdropClassName} fixed inset-0 z-[2500]`}
        ...
        <section role="dialog" aria-modal="true" aria-labelledby="theme-selector-title"
            ...
            <header
                ...
                <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                    {view === 'detail' && (<button type="button" onClick={() => setFocusSidebarOpen(true)}
                    ...
                    <div className="flex p-0.5 gap-0.5 rounded-xl border border-[var(--border)] bg-[var(--background)]">
                        {([
                            ...
                            <button type="button" onClick={() => onSetThemeMode(mode)}
                                    ...
                                    className={clsx('size-8 sm:size-9 rounded-lg flex items-center justify-center transition-all cursor-pointer', currentThemeMode === mode
                ...
                {view === 'gallery' ? (
                    <div className="modal-scroll-region h-full overflow-y-auto p-3 sm:p-4 md:p-5 scrollbar-thin">
                        ...
                        <div className="grid grid-cols-2 gap-2.5 sm:gap-3 md:grid-cols-2 lg:grid-cols-4">
                            {THEME_LIST.map((theme) => <ThemePreviewCard key={theme.name} theme={theme}
                                                                         selected={theme.name === selectedThemeName}
                                                                         resolvedThemeMode={resolvedThemeMode}
                                                                         onSelect={() => onSelectTheme(theme.name)}/>)}
                        </div>
                    </div>) : (<DetailedThemeView selectedTheme={selectedTheme} selectedThemeName={selectedThemeName}
                                                  currentThemeMode={currentThemeMode}
                                                  resolvedThemeMode={resolvedThemeMode} onSelectTheme={onSelectTheme}
                                                  onClose={requestClose} mobileSidebarOpen={focusSidebarOpen}
