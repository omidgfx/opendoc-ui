...
export default function PatternTesterModal({ pattern, onClose }: PatternTesterModalProps) {
    ...
    return (<div className={`${backdropClassName} fixed inset-0 z-[9999] backdrop-blur-[2px]`} style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={requestClose}>

            <div className="modal-surface w-full max-w-md rounded-2xl border shadow-2xl overflow-hidden bg-[var(--surface)] border-[var(--border)]" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="px-4 sm:px-5 py-2.5 sm:py-4 border-b shrink-0 flex items-center justify-between gap-2 border-[var(--border)] bg-[var(--background)] modal-header-mobile-pad">
                    ...
                    <Tip content="Close">
                        <button type="button" onClick={requestClose} className="w-8 h-8 rounded-lg hover:bg-[var(--surface-hover)] hover:text-[var(--primary-hover)] flex items-center justify-center text-sm cursor-pointer transition-colors text-[var(--text-muted)]">
                ...
                <div className="p-6 space-y-4 text-xs font-sans">
                    <div className="space-y-1.5">
                        <span className="font-semibold text-[var(--text-muted)] block uppercase tracking-wider text-[10px]">Active
                            Pattern</span>
                    ...
                    <div className="space-y-1.5">
                        <label htmlFor="regex-test-input" className="font-semibold text-[var(--text-muted)] block uppercase tracking-wider text-[10px]">
                            Test Input Value
                        </label>
                        <input id="regex-test-input" type="text" autoFocus placeholder="Type a value to test…" value={testValue} onChange={(e) => setTestValue(e.target.value)} className="w-full px-3 py-2 text-xs rounded-xl border outline-none focus:border-[var(--primary)] transition-colors font-mono bg-[var(--background)] border-[var(--border)] text-[var(--text)]"/>
            ...
            testValue ?
                ...
                <div className="px-5 py-3 border-t flex justify-end border-[var(--border)] bg-[var(--background)]">
                    ...
                    <button type="button" onClick={requestClose} className="px-4 py-1.5 text-[var(--primary-contrast)] font-semibold text-xs rounded-lg cursor-pointer hover:opacity-90 transition-all shadow-sm select-none bg-[var(--primary)]">
