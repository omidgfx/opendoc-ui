...
export default function ThemeNamePreview({ theme, mode }: {
    theme: AppTheme;
    mode: 'light' | 'dark';
}) {
    const palette = theme[mode];
    return (<div className="relative h-full overflow-hidden rounded-lg border p-3" style={{
            ...
            <div className="relative flex h-full flex-col justify-between">
                ...
                <div>
                    ...
                    <div className="mt-1 flex items-center gap-1">
                        {[palette.primary, palette.accent, palette.textHeading].map((color) => <span key={color} className="h-1.5 w-5 rounded-full" style={{ backgroundColor: color }}/>)}
                    </div>
                </div>
            </div>
        </div>);
