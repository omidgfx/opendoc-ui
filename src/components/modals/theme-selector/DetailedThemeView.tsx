...
export default function DetailedThemeView({ selectedTheme, selectedThemeName, currentThemeMode, resolvedThemeMode, onSelectTheme, onClose, mobileSidebarOpen, onCloseMobileSidebar }: {
    ...
    return (<div className="flex h-full min-h-0 relative">

            {mobileSidebarOpen && (<>
                    <div className="absolute inset-0 z-40 bg-black/40 md:hidden" onClick={onCloseMobileSidebar}/>
                    <aside className="absolute left-0 top-0 bottom-0 z-50 w-[78vw] max-w-[280px] overflow-y-auto border-r p-3 md:hidden scrollbar-thin bg-[var(--background)] border-[var(--border)] animate-in slide-in-from-left duration-200">
                        <div className="flex items-center justify-between pb-2 mb-2 border-b border-[var(--border)]">
                            <span className="text-[9px] font-black uppercase tracking-[0.18em] text-[var(--text-muted)]">All
                                themes · {THEME_LIST.length}</span>
                            <button onClick={onCloseMobileSidebar} className="size-7 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--surface-hover)]">
                ...
                return (<button type="button" key={theme.name} onClick={() => {
                        onSelectTheme(theme.name);
                        onCloseMobileSidebar();
                    }} aria-pressed={selected} className={clsx('flex w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition-all cursor-pointer', selected ? 'font-semibold' : 'border-transparent bg-transparent text-[var(--text)] hover:bg-[var(--surface-hover)]')} style={{
                        ...
                        borderColor: selected ? alpha(palette.primary, '55') : undefined
                                        ...
                                        <span className="relative h-8 w-8 shrink-0 overflow-hidden rounded-lg border" style={{ borderColor: palette.border }}>
                                            ...
                                            <span className="absolute left-1/2 top-1/2 h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border" style={{
            ...
            <aside className="hidden md:block w-60 shrink-0 overflow-y-auto border-r p-3 scrollbar-thin bg-[var(--background)] border-[var(--border)]" aria-label="Themes">
                <div className="px-2 pb-2 pt-1 text-[9px] font-black uppercase tracking-[0.18em] text-[var(--text-muted)]">All
                    themes · {THEME_LIST.length}</div>
                <div className="space-y-1">
                    {THEME_LIST.map((theme) => {
            const selected = theme.name === selectedThemeName;
            const palette = theme[resolvedThemeMode];
            return (<button type="button" key={theme.name} onClick={() => onSelectTheme(theme.name)} aria-pressed={selected} className={clsx('flex w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition-all cursor-pointer', selected ? 'font-semibold' : 'border-transparent bg-transparent text-[var(--text)] hover:bg-[var(--surface-hover)]')} style={{
                    color: selected ? palette.primary : undefined,
                    backgroundColor: selected ? alpha(palette.primary, '14') : undefined,
                    borderColor: selected ? alpha(palette.primary, '55') : undefined
                }}>
            ...
            <main className="modal-scroll-region min-w-0 flex-1 overflow-y-auto p-3 sm:p-4 md:p-5 scrollbar-thin">
                <div className="mb-4 flex items-center justify-between gap-3">
                    ...
                    <span className="inline-flex items-center gap-1.5 rounded-full border px-2 sm:px-3 py-1 sm:py-1.5 text-[9px] sm:text-[10px] font-bold shrink-0" style={{
            ...
            backgroundColor: alpha(selectedTheme[resolvedThemeMode].primary, '12')
                        ...
                        <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: selectedTheme[resolvedThemeMode].primary }}/>
                        Active
