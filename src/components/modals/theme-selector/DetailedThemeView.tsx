                    ...
                    {THEME_LIST.map((theme) => {
                        ...
                        return (<button type="button" key={theme.name} onClick={() => {
                            ...
                            onCloseMobileSidebar();
                                        ...
                                        className={clsx('flex w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition-all cursor-pointer', selected ? 'font-semibold' : 'border-transparent bg-transparent text-[var(--text)] hover:bg-[var(--surface-hover)]')}
                                        style={{
        ...
        <aside
            ...
            <div className="space-y-1">
                {THEME_LIST.map((theme) => {
                    const selected = theme.name === selectedThemeName;
                    const palette = theme[resolvedThemeMode];
                    return (<button type="button" key={theme.name} onClick={() => onSelectTheme(theme.name)}
                                    aria-pressed={selected}
                                    className={clsx('flex w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition-all cursor-pointer', selected ? 'font-semibold' : 'border-transparent bg-transparent text-[var(--text)] hover:bg-[var(--surface-hover)]')}
                                    style={{
                                        color: selected ? palette.primary : undefined,
