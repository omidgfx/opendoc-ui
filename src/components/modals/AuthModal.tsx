...
export default function AuthModal({ isOpen, onClose, spec, activeAuth, onSave }: AuthModalProps) {
    ...
    if (!shouldRender)
        return null;
    return (<div className={`${backdropClassName} fixed inset-0 z-[1500] bg-black/45 backdrop-blur-[2px]`} style={{ backgroundColor: 'rgba(0,0,0,.45)' }} onMouseDown={event => {
            ...
            <div className="modal-surface flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] text-[var(--text)] shadow-xl">
                <header className="flex items-center justify-between gap-3 border-b border-[var(--border)] bg-[var(--background)] px-5 py-4">
                    <div className="flex min-w-0 items-center gap-3"><span className="hidden size-9 shrink-0 items-center justify-center rounded-xl bg-[var(--primary)]/10 text-[var(--primary)] sm:flex"><i className="ph-fill ph-lock-key text-[18px]"/></span>
                        <div className="min-w-0"><h3 className="truncate text-sm font-bold text-[var(--text-heading)]">Configure
                            Authentication</h3><p className="mt-0.5 truncate text-[10px] text-[var(--text-muted)]">Security scheme IDs and
                            composed requirements are preserved.</p></div>
                    </div>
                    <Tip content="Close">
                        <button type="button" onClick={requestClose} className="flex size-8 items-center justify-center rounded-lg text-[var(--text-muted)] hover:bg-[var(--surface-hover)] cursor-pointer">
                ...
                <div className="modal-scroll-region min-h-0 space-y-4 overflow-y-auto px-4 sm:px-5 py-4 sm:py-5 scrollbar-thin">
                    <div>
                        <label className="mb-2 block text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Security
                            requirement</label>
                        <CustomDropdown value={selectedRequirement} onChange={setSelectedRequirement} options={options.map(option => ({ value: option.id, label: option.label }))} icon="ph ph-shield" className="w-full"/>
                    ...
                    {schemeIds.length === 0 && <div className="rounded-xl border border-[var(--border)] bg-[var(--background)] p-3 text-xs leading-relaxed text-[var(--text-muted)]">No
            ...
            return <div key={id} className="space-y-3 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4">
                            <div className="flex items-start justify-between gap-2">
                                <div>
                                    <div className="text-xs font-bold text-[var(--text-heading)]">{scheme ? getAuthSchemeLabel(id, scheme) : credential.type}</div>
                                    <div className="mt-0.5 font-mono text-[9px] text-[var(--text-muted)]">scheme
                                        id: {id}</div>
                                </div>
                                <span className="rounded bg-[var(--primary)]/10 px-2 py-1 text-[9px] font-bold uppercase text-[var(--primary)]">{credential.type}</span>
                            </div>
                            {(credential.type === 'apiKey' || credential.type === 'cookie') && <>
                                <label className="block space-y-1"><span className="block text-[10px] font-semibold text-[var(--text-muted)]">{isCookie ? 'Cookie name' : 'Key name'}</span><input className={fieldClass} value={credential.name || scheme?.name || ''} onChange={event => updateCredential(id, {
                        name: event.target.value,
                        in: credential.in || scheme?.in
                    })} readOnly={Boolean(scheme?.name)}/></label>
                                <label className="block space-y-1"><span className="block text-[10px] font-semibold text-[var(--text-muted)]">{isCookie ? 'Cookie value (agent mode only)' : 'Key value'}</span><input type="password" className={fieldClass} value={credential.value || ''} onChange={event => updateCredential(id, { value: event.target.value })} placeholder={isCookie ? 'Not injected by browser fetch' : 'Secret value'}/></label>
                                {isCookie &&
                    ...
                    <label className="block space-y-1"><span className="block text-[10px] font-semibold text-[var(--text-muted)]">Access
                                    token</span><input type="password" className={fieldClass} value={credential.value || ''} onChange={event => updateCredential(id, { value: event.target.value })} placeholder="Bearer access token"/><span className="block text-[10px] text-[var(--text-muted)]">OAuth authorization-code,
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><label className="space-y-1"><span className="block text-[10px] font-semibold text-[var(--text-muted)]">Username</span><input className={fieldClass} value={credential.username || ''} onChange={event => updateCredential(id, { username: event.target.value })}/></label><label className="space-y-1"><span className="block text-[10px] font-semibold text-[var(--text-muted)]">Password</span><input type="password" className={fieldClass} value={credential.password || ''} onChange={event => updateCredential(id, { password: event.target.value })}/></label>
                                </div>}
                        </div>;
        })}
                </div>
                ...
                <footer className="flex items-center justify-end gap-2 border-t border-[var(--border)] bg-[var(--background)] px-5 py-3">
                    <button type="button" onClick={requestClose} className="rounded-lg border border-[var(--border)] px-4 py-2 text-xs font-semibold text-[var(--text-heading)] hover:bg-[var(--surface-hover)] cursor-pointer">Cancel
                    </button>
                    <button type="button" onClick={save} className="rounded-lg bg-[var(--primary)] px-4 py-2 text-xs font-semibold text-[var(--primary-contrast)] hover:bg-[var(--primary-hover)] cursor-pointer">Apply
                    </button>
                </footer>
            </div>
        </div>);
