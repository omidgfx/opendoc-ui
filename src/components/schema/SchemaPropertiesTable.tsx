...
export default function SchemaPropertiesTable({ properties, schema, resolveReference, getRefName, onPushSchema, onViewExample, onTestPattern, useModal = false, inspectName = null }: SchemaPropertiesTableProps) {
    ...
    const mapValueLabel = (additionalProperties: any): string => {
        ...
        if (additionalProperties.$ref)
            return getRefName(additionalProperties.$ref);
        const t = Array.isArray(additionalProperties.type)
            ? additionalProperties.type.find((x: string) => x !== 'null')
            : additionalProperties.type;
        if (t === 'array') {
            if (additionalProperties.items?.$ref)
                return `Array<${getRefName(additionalProperties.items.$ref)}>`;
        ...
        if (t)
            return additionalProperties.format ? `${t} (${additionalProperties.format})` : `${t}`;
        return 'any';
    };
    const renderSchemaType = (prop: any): React.ReactNode => {
        ...
        if (prop.$ref) {
            ...
            return (<Tip content={`Inspect schema: ${refName}`}>
                    <button onClick={() => onPushSchema(refName)} className="text-[var(--primary)] hover:underline font-semibold text-xs text-left inline-flex items-center gap-1 cursor-pointer">
        ...
        if (prop.oneOf && Array.isArray(prop.oneOf)) {
            return (<div className="flex flex-col gap-1 items-start">
                    ...
                    <div className="flex flex-col flex-wrap gap-1.5">
                        {prop.oneOf.map((sub: any, sIdx: number) => <div key={sIdx}>
    ...
    if (Object.keys(properties).length === 0) {
        return (<p className="text-xs italic py-4 text-[var(--text-muted)]">
                No properties specified for this schema.
            </p>);
    }
    return (<div className="border rounded-xl overflow-hidden mt-2 mb-3 border-[var(--border)] bg-[var(--background)]">
                    ...
                    <tr className={"whitespace-nowrap brightness-95 bg-[var(--surface-hover)]"}>
                        ...
                        <th className="px-3 w-full py-2.5 font-semibold text-[10px] uppercase tracking-wider text-[var(--text-heading)]" style={{ width: '100%' }}>
                            ...
                            <div className={'flex justify-between'}>
                                ...
                                {useModal && (inspectName ?? schemaName) && (<Tip content={`Inspect ${inspectName ?? schemaName} schema`}>
                                        <button type="button" onClick={() => onPushSchema(inspectName ?? schemaName!)} className="sm:px-2 px-1.5 py-1 rounded-md text-[10px] font-sans flex items-center gap-1 transition-all cursor-pointer border hover:bg-[var(--background)] bg-[var(--surface)] border-[var(--border)] text-[var(--text-muted)]">
            ...
            return (<tr key={name} className="hover:bg-[var(--text-muted)]/5 transition-colors align-top border-b last:border-b-0 border-b-[var(--border)]">
                                <td className="px-3 py-2.5 font-mono font-bold text-[var(--text-heading)] whitespace-nowrap">
                                    <div className={'flex items-start gap-1'}>
                                        <span className="break-all">{name}</span>
                                        {isRequired && (<Tip content="Required field">
                    ...
                    <div className="text-[10px] font-mono flex items-center gap-1 mt-0.5">
                                                ...
                                                <button type="button" onClick={() => onTestPattern(pattern)} className="px-1 whitespace-nowrap py-0.5 text-[9px] font-bold text-[var(--primary)] bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 border border-[var(--primary)]/20 hover:underline inline-flex items-center gap-0.5 rounded cursor-pointer transition-colors">
                                ...
                                <td className="px-3 py-2.5 whitespace-nowrap">
                                    <div className="flex flex-col gap-1.5">
                                        {isComplexType && (<Tip content="Generate simulated example for this sub-schema">
                                                <button type="button" onClick={() => onViewExample(name, pVal)} className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 text-[var(--primary)] font-bold border border-[var(--primary)]/20 text-[10px] cursor-pointer transition-all select-none w-fit shrink-0">
                    ...
                    <div className="text-[10px] font-mono opacity-75">
                                                <span className="text-[var(--text-muted)] mr-1">{pVal.example !== undefined ? 'ex:' : 'def:'}</span>
                    ...
                    <span className="text-[var(--text-muted)] italic text-[11px]">No
                                            description</span>}
                                    {pVal.externalDocs && pVal.externalDocs.url &&
                    <div className="mt-1.5">
                                            <a href={pVal.externalDocs.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-2 py-0.5 text-[9px] font-bold text-[var(--primary)] bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 border border-[var(--primary)]/20 rounded cursor-pointer transition-colors">
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1 items-center">
                                            <span className="font-semibold text-[10px] text-[var(--text-muted)]">
                                                Enum:</span>
                                            {pVal.enum.map((vEnum: any) => <span key={vEnum} className="px-1.5 py-0.5 rounded text-[10px] font-mono border bg-[var(--background)] border-[var(--border)] text-[var(--text-muted)]">
