                    ...
                    {allMethods.map((method: string) => {
                        ...
                        return (
                            <button key={method} onClick={() => handleToggleMethod(method)}
                                ...
                                <span
                                    className={clsx('w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 transition-all text-[8.5px] font-bold',
                    ...
                    {[
                        ...
                        return (
                            <button key={opt.label} onClick={() => setOnlyProtected(opt.value as any)}
                                ...
                                <span
                                    className={clsx('size-3.5 rounded-full border flex items-center justify-center shrink-0 transition-all bg-[var(--text)]/5',
                    ...
                    {allTags.map((tag: string) => {
                        ...
                        return (
                            <button key={tag} onClick={() => handleToggleTag(tag)}
                                ...
                                <span
                                    className={clsx('w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 transition-all text-[8px] font-bold',
                                        isChecked ? 'bg-[var(--primary)] border-[var(--primary)] text-[var(--primary-contrast)]' : 'border-[var(--border)] bg-[var(--text)]/5')}>
                                    {isChecked && <i className="ph ph-check"></i>}
                                </span>
        ...
        Object.entries(spec.paths).forEach(([pathStr, pathItem]) => {
            Object.entries(pathItem).forEach(([methodStr, operation]) => {
                ...
                if (query) {
                    ...
                    // Every term must occur somewhere in the endpoint data. Separators
                    // make queries such as `some_text` behave like `some text`.
                            ...
                            <div key={`${method}-${path}`} className="animate-fade-in min-w-0 w-full max-w-lg">
                                <div className="flex items-center gap-2 mb-0.5 group min-w-0 w-fit">
                                    <div className="inline-flex items-center gap-2 cursor-pointer flex-1 min-w-0"
                                         ...
                                         onDoubleClick={(e) => {
                                             // Double-click opens in permanent tab (same as middle-click)
                                             e.preventDefault();
                                             e.stopPropagation();
                                ...
                                {operation.description ? (
                                    ...
                                    <p className="text-[12px] italic leading-relaxed text-[var(--text-muted)]">No
            ...
            {filtersModalOpen && isMobile && (
                <div
                    ...
                    <div
                        ...
                        <div
                            className="px-4 py-3 border-t flex gap-2 shrink-0 bg-[var(--background)] border-[var(--border)]">
                            {hasActiveFilters && (
                                <button onClick={handleClearFilters}
