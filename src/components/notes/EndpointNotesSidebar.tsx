...
export default function EndpointNotesSidebar({
    ...
    const panel = (
        <aside
            ...
            // Docked, the panel takes its own column so notes and content are
            // read side by side. Where there is no room for two columns it
            // slides over the content instead, like the mobile sidebar, and
            // covers the pinned response navigator (z-40) with it.
            className={clsx(
                'flex h-full min-h-0 flex-col border-l border-[var(--border)] bg-[var(--surface)]',
                overlay
                    ? [
                          'absolute inset-y-0 right-0 z-50 max-w-[86vw] transition-transform duration-300 ease-out',
                          open
                              ? 'translate-x-0 shadow-[-12px_0_28px_rgba(0,0,0,0.18)]'
        ...
        >
            {!overlay && (
                <div
                    ...
                    className={clsx(
                        'absolute inset-y-0 left-0 z-40 w-[4px] cursor-col-resize transition-colors select-none outline-none focus:bg-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/30',
            ...
            <header className="flex shrink-0 items-start justify-between gap-2 border-b border-[var(--border)] bg-[var(--background)] px-4 py-3">
                <div className="min-w-0">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="truncate text-xs font-extrabold text-[var(--text-heading)]">Endpoint Notes</h2>
                        <span className="text-[10px] font-bold text-[var(--text-muted)]">{endpointNotes.length}</span>
                    </div>
    ...
    // The column itself opens and closes, so the content beside it moves with
    // the panel instead of disappearing underneath it.
    return (
        <div
            data-endpoint-notes-dock
            className={clsx('h-full shrink-0 overflow-hidden', !dragging && 'transition-[width] duration-300 ease-out')}
