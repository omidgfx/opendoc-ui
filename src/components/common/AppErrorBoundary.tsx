...
export default class AppErrorBoundary extends React.Component<AppErrorBoundaryProps, AppErrorBoundaryState> {
    ...
    render() {
        ...
        return (
            <main
                className="flex min-h-screen items-center justify-center bg-[var(--background,#f8fafc)] p-6 text-[var(--text,#334155)]">
                <section
                    className="w-full max-w-2xl rounded-2xl border border-[var(--border,#d5dbe5)] bg-[var(--surface,#fff)] p-6 shadow-xl">
                    <div className="flex items-start gap-4">
                        <span
                            className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[#ef4444]/10 text-[#dc2626]"><i
                            className="ph ph-warning-octagon text-[22px]"/></span>
                        <div className="min-w-0 flex-1"><h1
                            className="text-lg font-extrabold text-[var(--text-heading,#172033)]">OpenDoc UI needs to
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                        ...
                        <a href={this.reportUrl()} target="_blank" rel="noreferrer"
                           className="rounded-xl border border-[var(--border,#d5dbe5)] px-4 py-2.5 text-xs font-bold text-[var(--text-heading,#172033)] hover:bg-[var(--surface-hover,#f1f5f9)] cursor-pointer"><i
                            className="ph ph-github-logo me-1.5"/>Report on GitHub</a></div>
                    <details
                        className="mt-5 rounded-xl border border-[var(--border,#d5dbe5)] bg-[var(--background,#f8fafc)] p-3">
                        <summary
                            className="cursor-pointer text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted,#64748b)]">Error
                            details
                        </summary>
                        <pre
                            className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap break-words font-mono text-[10px] leading-relaxed text-[#b91c1c]">{errorDetails || 'No stack details were provided by the browser.'}</pre>
                    </details>
                </section>
            </main>
        );
