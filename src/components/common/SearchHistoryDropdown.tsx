...
type SearchHistoryDropdownProps = {
    /** Spec key that owns the history (per-spec storage). */
    specKey: string;
    query: string;
...
/**
 * Google-style search history popover, stored per spec in localStorage.
 * Every time the input is focused with history available, this shows the
 * recent searches with per-item delete (x) and a Clear button at the top.
 */
export default function SearchHistoryDropdown({specKey, query, onPick, onClose}: SearchHistoryDropdownProps) {
    const [items, setItems] = useState<string[]>(() =>
        specStorage.getJSON<string[]>(specKey, 'search_history', [], (v) => Array.isArray(v) && v.every((x) => typeof x === 'string')),
    );
    ...
    // Keep the list in sync if history changes elsewhere (e.g. a new search was saved).
    useEffect(() => {
    ...
    return (
        ...
        >
            <div
                className="flex items-center justify-between px-3 py-2 border-b border-[var(--border)] bg-[var(--background)]">
