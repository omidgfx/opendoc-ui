...
export default function CustomDropdown({ value, onChange, options, icon, className = '', placeholder = 'Select...' }: CustomDropdownProps) {
    ...
    const menuContent = isOpen && (<div ref={menuRef} className="fixed z-[999999] bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-2xl py-1 text-sm min-w-[180px] overflow-hidden text-[var(--text)]" style={{
            top: position.top,
            left: position.left,
    ...
    return (<div ref={triggerRef} className={`relative ${className}`}>
            <div onClick={toggle} className="flex items-center justify-between gap-2 px-3 py-1.5 text-xs rounded-lg border bg-[var(--background)] border-[var(--border)] cursor-pointer hover:border-[var(--primary)]/50 transition-all select-none">
