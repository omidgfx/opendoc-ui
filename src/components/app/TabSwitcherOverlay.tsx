...
export default function TabSwitcherOverlay({ open, tabs, activeTabId, selectedIndex, onCancel, onSelect, }: TabSwitcherOverlayProps) {
    if (!open || tabs.length <= 1)
        return null;
    return (<div className="modal-backdrop fixed inset-0 z-[6000] bg-black/40 backdrop-blur-[2px] md:!items-start md:pt-[16vh]" onMouseDown={event => {
            ...
            const viewTabMeta = tab.kind && tab.kind !== 'endpoint'
                ? VIEW_TAB_META[tab.kind as ViewTabKind]
                : null;
            return (<button key={tab.id} type="button" onClick={() => onSelect(tab.id)} className={clsx('w-full flex items-center gap-3 px-3 py-2 text-left transition-colors cursor-pointer', selected ? 'bg-[var(--primary)]/10' : 'hover:bg-[var(--surface-hover)]')}>
                                {viewTabMeta ? (<span className="shrink-0 w-7 h-7 rounded-lg border border-[var(--border)] flex items-center justify-center text-[var(--primary)] bg-[var(--background)]">
                                        <i className={`${viewTabMeta.icon} text-[14px]`}/>
                                    </span>) : (<MethodBadge method={tab.method} size="xs" className="shrink-0 w-10 h-4"/>)}
                                <span className={clsx('flex-1 min-w-0 truncate text-xs font-semibold', selected ? 'text-[var(--text-heading)]' : 'text-[var(--text)]')}>
                                    {tab.label}
                                </span>
                                {current && (<span className="shrink-0 inline-flex items-center gap-1 text-[8px] font-black uppercase tracking-wider text-[var(--primary)]">
                                        <i className="ph-fill ph-dot-outline text-[10px]"/>
                                        Current
                                    </span>)}
                                {tab.isPreview && (<span className="shrink-0 text-[8px] font-black uppercase tracking-wider text-[var(--text-muted)]">Preview</span>)}
                                {selected &&
