...
export default function TabSwitcherOverlay({
    ...
    return (<div
        ...
        <div
            className="modal-surface w-[440px] max-w-[92vw] rounded-2xl border shadow-2xl overflow-hidden bg-[var(--surface)] border-[var(--border)]">
            <div
            ...
            <div className="py-1.5 max-h-[52vh] overflow-y-auto scrollbar-thin">
                {tabs.map((tab, index) => {
                    ...
                    const viewTabMeta = tab.kind && tab.kind !== 'endpoint'
                        ...
                        : null;
                    return (<button key={tab.id} type="button" onClick={() => onSelect(tab.id)}
                                    className={clsx('w-full flex items-center gap-3 px-3 py-2 text-left transition-colors cursor-pointer', selected ? 'bg-[var(--primary)]/10' : 'hover:bg-[var(--surface-hover)]')}>
                        {viewTabMeta ? (<span
                        ...
                        <span
                            className={clsx('flex-1 min-w-0 truncate text-xs font-semibold', selected ? 'text-[var(--text-heading)]' : 'text-[var(--text)]')}>
                            {tab.label}
                        </span>
                        {current && (<span
