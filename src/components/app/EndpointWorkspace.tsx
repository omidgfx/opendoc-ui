...
export default function EndpointWorkspace({ spec, endpoint, parsableKey, selectedTab, setSelectedTab, activeSplitPane, setActiveSplitPane, splitContainerRef, docsPaneWidth, isSplitDragging, onSplitResizeMouseDown, isMobile, activeAuth, selectedServer, resolvedThemeMode, activeResponseCode, setActiveResponseCode, currentResponse, onResponseChange, onClearResponse, onOpenSchema, onGenerateCode, }: EndpointWorkspaceProps) {
    ...
    const docs = (<ViewTab key={`${endpoint.path}-${endpoint.method}`} spec={spec} path={endpoint.path} method={endpoint.method} operation={operation} onOpenSchemaModal={onOpenSchema} activeAuth={activeAuth} activeResponseCode={activeResponseCode} onSelectResponseCode={setActiveResponseCode} parsableKey={parsableKey} isActive={docsActive}/>);
    const runner = (<ExamineTab spec={spec} path={endpoint.path} method={endpoint.method} operation={operation} activeAuth={activeAuth} selectedServer={selectedServer} parsableKey={parsableKey} themeMode={resolvedThemeMode} initialResponse={currentResponse} isActive={runnerActive} onResponseChange={onResponseChange} onClearResponse={onClearResponse}/>);
    return (<div className="flex-1 flex flex-col h-full overflow-hidden min-w-0">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    <div className="flex p-0.5 gap-1 rounded-lg border text-xs border-[var(--border)] bg-[var(--background)]">
                        <Tip content="View Documentation">
                            <button onClick={() => setSelectedTab('docs')} aria-pressed={selectedTab === 'docs'} className={clsx('px-2.5 sm:px-3 py-1.5 gap-1.5 flex items-center rounded-md font-semibold transition-all cursor-pointer text-xs', selectedTab === 'docs' ? 'bg-[var(--method-get)] shadow-sm text-[var(--method-get-contrast)]' : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)]')}>
                        ...
                        <Tip content="API Runner">
                            <button onClick={() => setSelectedTab('examine')} aria-pressed={selectedTab === 'examine'} className={clsx('px-2.5 sm:px-3 py-1.5 gap-1.5 flex items-center rounded-md font-semibold transition-all cursor-pointer text-xs', selectedTab === 'examine' ? 'bg-[var(--method-delete)] shadow-sm text-[var(--method-delete-contrast)]' : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)]')}>
                        ...
                        <Tip content="Split View (Side-by-Side)">
                            <button onClick={() => setSelectedTab('both')} aria-pressed={selectedTab === 'both'} className={clsx('px-2.5 sm:px-3 py-1.5 gap-1.5 flex items-center rounded-md font-semibold transition-all cursor-pointer text-xs', selectedTab === 'both' ? 'bg-[var(--primary)] shadow-sm text-[var(--primary-contrast)]' : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)]')}>
                    ...
                    <Tip content="Generate Fetch/Axios snippets and TypeScript models">
                        <button onClick={onGenerateCode} className="size-8.5 border hover:bg-[var(--surface-hover)] rounded-lg text-xs font-bold flex justify-center items-center transition-colors cursor-pointer border-[var(--border)] text-[var(--text-heading)] shrink-0">
            ...
            <div className="flex-1 overflow-hidden h-full min-h-0">
                {selectedTab === 'both' ? (isMobile ? (<div ref={splitContainerRef} className="flex flex-col h-full w-full min-h-0 min-w-0 gap-1.5 p-1.5 overflow-y-auto scrollbar-thin">
                        ...
                        </div>) : (<div ref={splitContainerRef} className="flex h-full w-full min-h-0 min-w-0 gap-0.5 p-1.5">
                ...
                flex: docsPaneWidth >= 0 ? '0 0 auto' : '1 1 0%',
                            ...
                            <div onMouseDown={onSplitResizeMouseDown} className={clsx('w-1.5 shrink-0 h-full rounded-full cursor-col-resize transition-colors select-none', isSplitDragging ? 'bg-[var(--primary)]' : 'bg-transparent hover:bg-[var(--primary)]/60')}/>
                            <div className="h-full min-w-0 flex-1 overflow-hidden">
                                <FocusPane active={activeSplitPane === 'examine'} onActivate={() => setActiveSplitPane('examine')}>
                                    {runner}
                                </FocusPane>
                            </div>
                        </div>)) : selectedTab === 'docs' ? docs : runner}
            </div>
        </div>);
}
