...
export default function EndpointWorkspace({
    ...
    const docs = (
        <ViewTab key={`${endpoint.path}-${endpoint.method}`} spec={spec} path={endpoint.path} method={endpoint.method}
                 operation={operation} onOpenSchemaModal={onOpenSchema} activeAuth={activeAuth}
    ...
    return (<div className="flex-1 flex flex-col h-full overflow-hidden min-w-0">
                ...
                <div
                    ...
                    <Tip content="View Documentation">
                        <button onClick={() => setSelectedTab('docs')} aria-pressed={selectedTab === 'docs'}
                                className={clsx('px-2.5 sm:px-3 py-1.5 gap-1.5 flex items-center rounded-md font-semibold transition-all cursor-pointer text-xs', selectedTab === 'docs' ? 'bg-[var(--method-get)] shadow-sm text-[var(--method-get-contrast)]' : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)]')}>
                    ...
                    <Tip content="API Runner">
                        <button onClick={() => setSelectedTab('examine')} aria-pressed={selectedTab === 'examine'}
                                className={clsx('px-2.5 sm:px-3 py-1.5 gap-1.5 flex items-center rounded-md font-semibold transition-all cursor-pointer text-xs', selectedTab === 'examine' ? 'bg-[var(--method-delete)] shadow-sm text-[var(--method-delete-contrast)]' : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)]')}>
                    ...
                    <Tip content="Split View (Side-by-Side)">
                        <button onClick={() => setSelectedTab('both')} aria-pressed={selectedTab === 'both'}
                                className={clsx('px-2.5 sm:px-3 py-1.5 gap-1.5 flex items-center rounded-md font-semibold transition-all cursor-pointer text-xs', selectedTab === 'both' ? 'bg-[var(--primary)] shadow-sm text-[var(--primary-contrast)]' : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)]')}>
        ...
        <div className="flex-1 overflow-hidden h-full min-h-0">
            {selectedTab === 'both' ? (isMobile ? (<div ref={splitContainerRef}
