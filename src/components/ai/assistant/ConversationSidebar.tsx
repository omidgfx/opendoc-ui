...
export default function ConversationSidebar({
    ...
    return (
        <aside
            className={clsx(
                'hidden shrink-0 flex-col border-r border-[var(--border)] bg-[var(--background)] transition-all duration-300 md:flex',
        ...
        >
            <div className="flex h-14 min-h-14 items-center justify-between gap-2 border-b border-[var(--border)] px-3">
                <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-[var(--text-muted)]">
                    <i className="ph-fill ph-chats-circle text-[15px] text-[var(--primary)]" />
                    Conversations
                </span>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-2 scrollbar-thin">
                {conversations.length === 0 && (
                    <p className="px-2 py-4 text-[10px] leading-relaxed text-[var(--text-muted)]">
                        No saved conversations. Use the plus button or start typing below.
                    </p>
                )}
                {conversations.map(conversation => (
                    <button
                        ...
                        className={clsx(
                            'group mb-1 flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left transition-colors cursor-pointer',
                    ...
                    >
                        ...
                        <span className="min-w-0 flex-1 truncate text-[11px] font-semibold">{conversation.title}</span>
                        <span
                            role="button"
                            tabIndex={0}
