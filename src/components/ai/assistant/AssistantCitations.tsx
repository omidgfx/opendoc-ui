...
export default function AssistantCitations({ citations, onOpenEndpoint }: AssistantCitationsProps) {
    if (!citations || citations.length === 0)
        return null;
    return (<div className="mt-3 flex flex-wrap gap-1.5 border-t border-[var(--border)]/70 pt-2.5">
            ...
            return (<button key={source.id} type="button" onClick={() => isEndpoint && onOpenEndpoint(source.path!, source.method!.toLowerCase())} className={clsx('rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-[9px] font-semibold text-[var(--text-muted)] transition-colors', isEndpoint ? 'cursor-pointer hover:border-[var(--primary)]/50 hover:text-[var(--primary)]' : 'cursor-default')}>
                        {source.label}
                    </button>);
        })}
        </div>);
