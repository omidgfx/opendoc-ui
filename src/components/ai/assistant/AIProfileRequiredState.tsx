...
export default function AIProfileRequiredState({ onOpenSettings }: AIProfileRequiredStateProps) {
    return (<div className="flex h-full min-h-0 w-full items-center justify-center overflow-y-auto bg-[var(--surface)] p-6">
            <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--background)] p-7 text-center shadow-sm">
                ...
                <button type="button" onClick={onOpenSettings} className="mt-5 rounded-xl bg-[var(--primary)] px-4 py-2.5 text-xs font-bold text-[var(--primary-contrast)] hover:brightness-110 cursor-pointer">
                    <i className="ph ph-plus me-1.5"/>Create profile
                </button>
            </div>
        </div>);
}
