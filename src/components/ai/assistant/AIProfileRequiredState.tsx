interface AIProfileRequiredStateProps {
    onOpenSettings: () => void;
    /** Managed mode with a backend that is not ready yet: no config CTA. */
    managedStartup?: boolean;
}

export default function AIProfileRequiredState({onOpenSettings, managedStartup = false}: AIProfileRequiredStateProps) {
    return (
        <div className="flex h-full min-h-0 w-full items-center justify-center overflow-y-auto bg-[var(--surface)] p-6">
            <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--background)] p-7 text-center shadow-sm">
                ...
                {managedStartup ? (
                    <>
                        <h1 className="mt-5 text-lg font-extrabold text-[var(--text-heading)]">AI is starting up</h1>
                        <p className="mt-2 text-xs leading-relaxed text-[var(--text-muted)]">
                            Your organization's assistant is managed on the backend and is not ready yet. It will be
                            available shortly — no setup is needed on your side.
                        </p>
                    </>
                ) : (
                    <>
                        ...
                        <p className="mt-2 text-xs leading-relaxed text-[var(--text-muted)]">
                            The AI Assistant needs a provider profile before it can answer questions. Choose a provider,
                            model, and transport to get started.
                        </p>
                        <button
                            type="button"
