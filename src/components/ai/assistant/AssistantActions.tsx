...
export default function AssistantActions({ text, onExecute }: AssistantActionsProps) {
    ...
    if (actions.length === 0)
        return null;
    return (<div className="mt-3 space-y-2 border-t border-[var(--border)]/70 pt-2.5">
            ...
            {actions.map((action, index) => (<button key={`${action.action}-${index}`} type="button" onClick={() => onExecute(action)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-[var(--primary)]/25 bg-[var(--primary)]/5 px-3 py-2 text-left text-[10px] font-bold text-[var(--primary)] hover:bg-[var(--primary)]/10 cursor-pointer">
