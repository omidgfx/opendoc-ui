...
export default function MobileConversationsModal({
                                                     visible,
                                                     backdropClassName,
                                                     conversations,
                                                     activeId,
    ...
    if (!visible)
        return null;
    return (<div className={`${backdropClassName} fixed inset-0 z-[5900] bg-black/50 backdrop-blur-[2px] md:hidden`}
                 onMouseDown={event => {
                     if (event.target === event.currentTarget)
                         onClose();
                 }}>
        <aside
            className="modal-surface modal-surface-stable flex w-full flex-col overflow-hidden border border-[var(--border)] bg-[var(--surface)] shadow-2xl">
            <header
                className="flex shrink-0 items-center justify-between border-b border-[var(--border)] bg-[var(--background)] px-4 py-3">
                <div><h2 className="text-sm font-extrabold text-[var(--text-heading)]">Conversations</h2><p
                    className="mt-0.5 text-[10px] text-[var(--text-muted)]">Saved for this specification</p></div>
            ...
            <div className="modal-scroll-region min-h-0 flex-1 space-y-1 overflow-y-auto p-2 scrollbar-thin">
                {conversations.length === 0 &&
                    <p className="px-3 py-8 text-center text-xs text-[var(--text-muted)]">No saved conversations
                        yet.</p>}
                {conversations.map(conversation => <div key={conversation.id}
                                                        className={clsx('flex items-center gap-2 rounded-xl px-2 py-1.5', conversation.id === activeId ? 'bg-[var(--primary)]/10' : 'hover:bg-[var(--surface-hover)]')}>
                    ...
                    <button type="button" onClick={() => onDelete(conversation)}
