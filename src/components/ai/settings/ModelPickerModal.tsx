...
export default function ModelPickerModal({
    ...
    return (
        <div
            className={`${backdropClassName} fixed inset-0 z-[6100] bg-black/45`}
            onMouseDown={event => {
            ...
            >
                <header className="flex items-center justify-between gap-3 border-b border-[var(--border)] bg-[var(--background)] px-4 py-3">
                    <div className="min-w-0">
                        <h3 id="ai-model-picker-title" className="text-sm font-extrabold text-[var(--text-heading)]">
                            Choose a model
                        </h3>
                        <p className="mt-0.5 truncate text-[10px] text-[var(--text-muted)]">
                            Search by model name or slug. Current: {currentModel || 'none'}
                        </p>
                    </div>
                    <div className="flex shrink-0 items-center gap-1.5">
                        ...
                        >
                            <i className={clsx('ph ph-arrows-clockwise text-[13px]', refreshing && 'animate-spin')} />
                ...
                <div className="space-y-2 border-b border-[var(--border)] p-3">
                    <div className="relative">
                        <i className="ph ph-magnifying-glass pointer-events-none absolute start-3 top-1/2 -translate-y-1/2 text-[13px] text-[var(--text-muted)]" />
                        <input
                            autoFocus
                            value={search}
                    ...
                    <div className="flex items-center gap-1 rounded-xl border border-[var(--border)] bg-[var(--background)] p-1">
                        {(['free', 'premium', 'all'] as const).map(filter => (
                            <button
                                ...
                                className={clsx(
                                    'flex-1 rounded-lg px-2 py-1.5 text-[9px] font-black uppercase tracking-wider transition-colors cursor-pointer',
                                    tier === filter
                ...
                <div className="modal-scroll-region min-h-0 flex-1 overflow-y-auto p-2 scrollbar-thin">
                    {models.length === 0 ? (
                        <p className="px-3 py-10 text-center text-xs text-[var(--text-muted)]">
                        ...
                        models.map(model => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <i className={selected ? 'ph ph-check text-[13px]' : 'ph ph-cpu text-[13px]'} />
                                    </span>
                                    <span className="min-w-0 flex-1">
                                        <span className="block truncate text-[11px] font-bold text-[var(--text-heading)]">
                                            <ModelSearchHighlight text={model.label} query={search} />
                                        </span>
                                        <span className="mt-0.5 block truncate font-mono text-[10px] text-[var(--text-muted)]">
                                            <ModelSearchHighlight text={model.id} query={search} />
                                        </span>
                                    </span>
                                    <span className="shrink-0 rounded-md border border-[var(--border)] px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider text-[var(--text-muted)]">
                                        {model.tier}
                                    </span>
                                </button>
                            );
