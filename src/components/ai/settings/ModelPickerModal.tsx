        ...
        <div role="dialog" aria-modal="true" aria-labelledby="ai-model-picker-title"
            ...
            <header
                ...
                <div className="min-w-0"><h3 id="ai-model-picker-title"
                                             className="text-sm font-extrabold text-[var(--text-heading)]">Choose a
                    model</h3><p className="mt-0.5 truncate text-[10px] text-[var(--text-muted)]">Search by model name
                    or slug. Current: {currentModel || 'none'}</p></div>
                <div className="flex shrink-0 items-center gap-1.5">
                    <button type="button" onClick={onRefresh} disabled={refreshing}
                        ...
                        <i className={clsx('ph ph-arrows-clockwise text-[13px]', refreshing && 'animate-spin')}/>{refreshing ? 'Refreshing…' : 'Refresh models'}
                    </button>
            ...
            <div className="space-y-2 border-b border-[var(--border)] p-3">
                <div className="relative"><i
                    ...
                    autoFocus value={search} onChange={event => onSearchChange(event.target.value)}
                ...
                <div
                    ...
                    {(['free', 'premium', 'all'] as const).map(filter => <button key={filter} type="button"
                                                                                 onClick={() => onTierChange(filter)}
                                                                                 className={clsx('flex-1 rounded-lg px-2 py-1.5 text-[9px] font-black uppercase tracking-wider transition-colors cursor-pointer', tier === filter ? 'bg-[var(--primary)] text-[var(--primary-contrast)]' : 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)]')}>{filter}</button>)}
                </div>
            </div>
            <div className="modal-scroll-region min-h-0 flex-1 overflow-y-auto p-2 scrollbar-thin">
                {models.length === 0 ?
                    <p className="px-3 py-10 text-center text-xs text-[var(--text-muted)]">No models match this
                        filter.</p> : models.map(model => {
                        const selected = model.id === currentModel;
                        return <button key={model.id} type="button" onClick={() => onSelect(model)}
