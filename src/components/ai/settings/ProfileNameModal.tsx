...
export default function ProfileNameModal({
    ...
    return (
        <div
            className={`${backdropClassName} fixed inset-0 z-[6200] bg-black/55 backdrop-blur-[2px]`}
            onMouseDown={event => {
            ...
            >
                <header className="flex items-center justify-between border-b border-[var(--border)] bg-[var(--background)] px-4 py-3">
                    <div className="flex items-center gap-2.5">
                        <span className="flex size-9 items-center justify-center rounded-xl bg-[var(--primary)]/10 text-[var(--primary)]">
                            <i className={rename ? 'ph ph-pencil-simple text-[17px]' : 'ph ph-user-plus text-[17px]'} />
                        </span>
                        <div>
                            <h3 className="text-sm font-extrabold text-[var(--text-heading)]">
                                {rename ? 'Rename assistant profile' : 'Create assistant profile'}
                            </h3>
                            <p className="mt-0.5 text-[10px] text-[var(--text-muted)]">
                                {rename ? 'Update the saved profile name.' : 'Choose a name before creating it.'}
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <footer className="flex justify-end gap-2 border-t border-[var(--border)] bg-[var(--background)] px-4 py-3">
                    ...
                    <button
                        type="submit"
                        disabled={!name.trim()}
                        className="whitespace-nowrap rounded-xl bg-[var(--primary)] px-4 py-2 text-xs font-bold text-[var(--primary-contrast)] hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40 cursor-pointer"
                    >
                        {rename ? 'Save name' : 'Create profile'}
                    </button>
                </footer>
            </form>
        </div>
