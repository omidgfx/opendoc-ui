...
export default function TemperatureSlider({ value, onChange }: TemperatureSliderProps) {
    ...
    return (<div className="rounded-xl border border-[var(--border)] bg-[var(--background)] px-3 py-3">
            <div role="slider" aria-label="Temperature" aria-valuemin={0} aria-valuemax={2} aria-valuenow={value} tabIndex={0} className="relative h-7 cursor-pointer touch-none select-none" onPointerDown={event => {
            ...
            if (event.key === 'End') {
                ...
                <div className="absolute left-0 top-1/2 h-2 -translate-y-1/2 rounded-full bg-[var(--primary)]" style={{ width: `${(value / 2) * 100}%` }}/>
                <div className="absolute top-1/2 size-5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-[var(--primary)] bg-[var(--surface)] shadow-md" style={{ left: `${(value / 2) * 100}%` }}/>
            </div>
