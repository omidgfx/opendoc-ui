...
export default function SettingsConfirmModal({
    ...
    return (<div className={`${backdropClassName} fixed inset-0 z-[6000] bg-black/55 backdrop-blur-[2px]`}
                 onMouseDown={event => {
        ...
        <section
            ...
            <header
                ...
                <span
                    className={clsx('flex size-10 shrink-0 items-center justify-center rounded-xl', item.tone === 'danger' ? 'bg-[var(--method-delete)]/10 text-[var(--method-delete)]' : 'bg-[var(--primary)]/10 text-[var(--primary)]')}><i
                    className={`ph ${item.icon} text-[18px]`}/></span>
                <h3 className="text-sm font-extrabold text-[var(--text-heading)]">{item.title}</h3>
            </header>
            <div className="px-4 py-4"><p
                className="text-[11px] leading-relaxed text-[var(--text-muted)]">{item.body}</p></div>
            <footer className="flex justify-end gap-2 border-t border-[var(--border)] bg-[var(--background)] px-4 py-3">
                ...
                <button type="button" onClick={onConfirm}
                        className={clsx('whitespace-nowrap rounded-xl px-4 py-2 text-xs font-bold hover:brightness-110 cursor-pointer', item.tone === 'danger' ? 'bg-[var(--method-delete)] text-[var(--method-delete-contrast)]' : 'bg-[var(--primary)] text-[var(--primary-contrast)]')}>{item.label}</button>
            </footer>
        </section>
    </div>);
}
