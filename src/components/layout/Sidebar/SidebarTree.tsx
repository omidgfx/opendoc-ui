    ...
    const render = (node: TreeNode, nodePath: string) => {
        ...
        const renderGuides = (i: number, onPath: boolean) => {
            ...
            return (<>
                ...
                <span className={clsx('absolute h-px', onPath ? 'bg-[var(--primary)]' : 'bg-[var(--text)]/25')}
                      style={{left: -GUIDE_X + 1, top: ROW_ELBOW_Y, width: ELBOW_W}} aria-hidden="true"/>

                <span className="absolute top-0 w-px bg-[var(--text)]/25"
                      style={{left: -GUIDE_X, ...(isLastRow ? {height: ROW_ELBOW_Y + 1} : {bottom: 0})}}
                      aria-hidden="true"/>

                {accent !== 'none' && (<span className="absolute top-0 w-px bg-[var(--primary)]"
        ...
        return (<div key={nodePath} className="relative animate-in fade-in duration-150">
            ...
            <button onClick={() => toggleNode(nodePath)}
                ...
                {!sidebarConfig.hideEndpointCount && (<span
                               ...
                               className={clsx('flex items-center w-full py-1.5 font-medium ps-2 pe-2 rounded-lg text-left transition-all cursor-pointer select-none min-w-0', isSelected ? 'bg-[var(--primary)]/90 text-[var(--primary-contrast)]' : 'bg-transparent text-[var(--text)] hover:bg-[var(--surface-hover)]')}>
                                <div className="flex items-center gap-1.5 min-w-0 w-full">
                                    <Tip content={ep.method.toUpperCase()} placement="top">
                                        <MethodBadge method={ep.method.toLowerCase()}
                                                     displayLabel={sidebarConfig.compactMethodNames ? compactMethodLabel(ep.method) : undefined}
                                                     size="xs"
                                                     className={clsx(sidebarConfig.compactMethodNames ? 'w-5 h-4 !px-0' : 'w-9 h-4', 'shrink-0', isSelected && '!bg-[var(--primary-contrast)]/20 !text-[var(--primary-contrast)] !border-[var(--primary-contrast)]/30')}/>
                                    </Tip>
                                    ...
                                    <div className="min-w-0 grow flex flex-col justify-center leading-[1.3333]">
                                        ...
                                        {sidebarConfig.displayRoutes && (<span
