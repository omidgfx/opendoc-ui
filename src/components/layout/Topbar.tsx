...
export default function Topbar({ parsables, selectedParsableKey, onSelectParsable, activeAuth, searchQuery, onSearchChange, onDownloadSpec, title, showSchemaExplorer, spec, selectedThemeName, onSelectTheme, isCollapsed, onToggleCollapse, onOpenMobileSidebar, onOpenAssistant, onOpenAuthModal, onOpenThemeModal, isLocalMode, canOpenLocal, onOpenLocalFile, onRefreshSpec, onReloadSpecification, onResetSpecification, onResetAllConfigurations, isRefreshingSpec, localHistory, onSelectHistoryEntry, onRemoveHistoryEntry, onClearHistory, localOpenError, onDismissLocalError, onSearchHasResults, hideSearch, }: TopbarProps & {
    ...
    const selectorButton = isLocalMode
        ? canOpenLocal && (<Tip content="Open a specification from your device">
            <button type="button" onClick={() => setShowSpecificationModal(true)} className="flex h-8 w-40 xl:w-48 items-center gap-2 rounded-lg border border-[var(--border)] px-3 text-left text-[var(--text-heading)] transition-all cursor-pointer hover:bg-[var(--surface-hover)]">
        ...
        : (<Tip content="Switch API specification">
                <button type="button" onClick={() => setShowSpecificationModal(true)} className="flex h-8 w-44 xl:w-56 items-center gap-2 rounded-lg border border-[var(--border)] px-3 text-left text-[var(--text-heading)] transition-all cursor-pointer hover:bg-[var(--surface-hover)]">
    ...
    return (<>
            <div className="app-topbar h-14 sm:h-16 border-b px-2 sm:px-3 flex items-center justify-between select-none shrink-0 font-sans z-30 bg-[var(--navbar)] border-[var(--border)] gap-2">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <Tip content={isMobile ? 'Open menu' : (isCollapsed ? 'Expand sidebar' : 'Collapse sidebar')} placement="bottom">
                        <button onClick={isMobile ? onOpenMobileSidebar : onToggleCollapse} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[var(--surface-hover)] transition-all cursor-pointer text-[var(--text-heading)] shrink-0">
                ...
                {isMobile && isLocalMode && canOpenLocal && (<Tip content="Open a specification from your device" placement="bottom">
                        <button type="button" onClick={() => setShowSpecificationModal(true)} className="h-8 px-2.5 rounded-lg border flex items-center justify-center gap-1.5 cursor-pointer border-[var(--border)] text-[var(--text-heading)] hover:bg-[var(--surface-hover)] shrink-0">
                ...
                {hasSpec && !showSchemaExplorer && !isMobile && !hideSearch && (<div className="hidden md:flex flex-1 items-center relative max-w-md min-w-0 select-none">
                        <input ref={searchInputRef} type="text" placeholder="Global Search (Ctrl+K)..." value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} onFocus={handleSearchFocus} onBlur={handleSearchBlur} onKeyDown={handleSearchKeyDown} className="w-full min-w-0 pl-9 pr-14 h-8 text-xs rounded-lg border outline-none focus:border-[var(--primary)] focus:bg-[var(--surface)] transition-all font-sans border-[var(--border)] text-[var(--text)] bg-[var(--background)]"/>
                        {searchFocused && selectedParsableKey && (<SearchHistoryDropdown key={searchHistoryVersion} specKey={selectedParsableKey} query={searchQuery} onPick={(q) => onSearchChange(q)} onClose={() => setSearchFocused(false)}/>)}
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-[var(--text-muted)]">
                            <i className="ph ph-magnifying-glass text-[14px]"></i>
                        </div>
                        {searchQuery ? (<button onClick={() => onSearchChange('')} className="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer text-[var(--text-muted)]">
                                <i className="ph ph-x text-[14px]"></i>
                            </button>) : (<div className="absolute inset-y-0 right-0 pr-1.5 flex items-center pointer-events-none select-none">
                ...
                <div className="flex items-center gap-1 shrink-0">
                    {hasSpec && (<Tip content="Open AI Assistant" placement="bottom">
                            <button type="button" onClick={onOpenAssistant} className="size-8 rounded-lg border flex items-center justify-center transition-colors cursor-pointer border-[var(--border)] text-[var(--primary)] hover:bg-[var(--surface-hover)]">
                    ...
                    {hasSpec && !showSchemaExplorer && isMobile && !hideSearch && (<Tip content="Search" placement="bottom">
                            <button onClick={() => setShowMobileSearch(v => !v)} className="size-8 rounded-lg flex items-center justify-center border cursor-pointer border-[var(--border)] text-[var(--text-heading)] hover:bg-[var(--surface-hover)]">
                    ...
                    {!isMobile && hasSpec && (<>
                            <Tip content={authConnected ? 'Authentication active — click to edit' : 'Configure authentication'}>
                                <button onClick={onOpenAuthModal} className="h-8 ps-2.5 pe-2 border cursor-pointer border-[var(--border)] text-xs font-semibold rounded-lg flex items-center gap-1.5 transition-all select-none hover:bg-[var(--surface-hover)]">
                            ...
                            <Tip content="Download raw specification">
                                <button onClick={onDownloadSpec} className="size-8 rounded-lg border flex items-center justify-center transition-colors cursor-pointer border-[var(--border)] text-[var(--text-heading)] hover:bg-[var(--surface-hover)]">
                    ...
                    {hasSpec && (<Tip content="Choose theme">
                            <button type="button" onClick={onOpenThemeModal} className="flex size-8 xl:w-40 items-center gap-2 rounded-lg border px-2 xl:px-3 text-left transition-all cursor-pointer border-[var(--border)] text-[var(--text-heading)] hover:bg-[var(--surface-hover)]">
            ...
            {showMobileSearch && !showSchemaExplorer && isMobile && (<div className="sm:hidden border-b px-3 py-2 flex items-center gap-2 bg-[var(--navbar)] border-[var(--border)]">
                    <div className="relative flex-1 min-w-0">
                        <input ref={searchInputRef} type="text" autoFocus placeholder="Search..." value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} onFocus={handleSearchFocus} onBlur={handleSearchBlur} onKeyDown={handleSearchKeyDown} className="w-full pl-9 pr-8 h-9 text-xs rounded-lg border outline-none focus:border-[var(--primary)] focus:bg-[var(--surface)] bg-[var(--background)] border-[var(--border)] text-[var(--text)]"/>
                        {searchFocused && selectedParsableKey && (<SearchHistoryDropdown key={searchHistoryVersion} specKey={selectedParsableKey} query={searchQuery} onPick={(q) => onSearchChange(q)} onClose={() => setSearchFocused(false)}/>)}
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-[var(--text-muted)]">
                            <i className="ph ph-magnifying-glass text-[14px]"></i>
                        </div>
                        {searchQuery && (<button onClick={() => onSearchChange('')} className="absolute inset-y-0 right-0 pr-2 flex items-center cursor-pointer text-[var(--text-muted)]">
                                <i className="ph ph-x text-[14px]"></i>
                            </button>)}
                    </div>
                    <button onClick={() => setShowMobileSearch(false)} className="text-xs font-semibold text-[var(--text-muted)] px-2 py-1 cursor-pointer shrink-0">Cancel
                    </button>
                </div>)}

            <ApiSpecificationSelectorModal isOpen={showSpecificationModal} specifications={parsables} selectedKey={selectedParsableKey} activeSpecification={spec} isLocalMode={isLocalMode} canOpenLocal={canOpenLocal} onOpenLocalFile={onOpenLocalFile} onReloadSpecification={onReloadSpecification} onResetSpecification={onResetSpecification} onResetAllConfigurations={onResetAllConfigurations} localHistory={localHistory} onSelectHistoryEntry={onSelectHistoryEntry} onRemoveHistoryEntry={onRemoveHistoryEntry} onClearHistory={onClearHistory} localOpenError={localOpenError} onDismissLocalError={onDismissLocalError} onSelect={(k) => {
            onSelectParsable(k);
