...
interface ViewTabProps {
    ...
    /** Key of the currently selected parsable/spec — used to namespace persisted scroll position. */
    parsableKey?: string;
...
export default function ViewTab({
                                    ...
                                    activeResponseCode, onSelectResponseCode, parsableKey = '', isActive = true,
                                }: ViewTabProps) {
    ...
    // Remembers where the reader left off inside this endpoint's documentation so
    // returning to it later resumes from the same spot. A `#response-*` fragment
    ...
    // ---------- Ctrl+Up / Ctrl+Down response navigation ----------
    // Ctrl+Down: close current response (if open) then jump to the next response code.
    // Ctrl+Up: same, but to the previous response code.
    // Ctrl+Shift+Down / Ctrl+Shift+Up: same navigation, but never closes the current response.
    useEffect(() => {
        if (!isActive) return;
        ...
        const handler = (e: KeyboardEvent) => {
            ...
            // Non-shift variants close the currently open response first (by simulating
            // the same click the user would perform on its header).
    ...
    const renderSchemaType = (prop: any): React.ReactNode => {
            ...
            return (
                <div className="flex flex-col gap-1.5 items-start">
                    <span
                        ...
                        Of:</span>
                    <div
                        className="flex p-0.5 rounded-lg border flex-wrap border-[var(--border)] bg-[var(--background)]">
        ...
        if (prop.type === 'array' && prop.items) {
            ...
            if (prop.items.oneOf || prop.items.anyOf) return <span
                className="text-xs font-sans">Array&lt;{renderSchemaType(prop.items)}&gt;</span>;
    ...
    const resolveProperties = (sObj: any, prefix = '', visited = new Set<string>()): { [name: string]: any } => {
        ...
        if (sObj.allOf && Array.isArray(sObj.allOf)) sObj.allOf.forEach((sub: any) => {
        ...
        if (sObj.oneOf && Array.isArray(sObj.oneOf)) sObj.oneOf.forEach((sub: any) => {
            props = {...props, ...resolveProperties(sub, prefix, new Set(visited))};
        });
        if (sObj.anyOf && Array.isArray(sObj.anyOf)) sObj.anyOf.forEach((sub: any) => {
            props = {...props, ...resolveProperties(sub, prefix, new Set(visited))};
        });
    ...
    const renderSchemaPropertiesTable = (schema: any, inspectName?: string | null) => {
        ...
        return (
            <SchemaPropertiesTable properties={properties} schema={schema} resolveReference={resolveReference}
        ...
        const generateMock = (s: any, depth = 0, visited = new Set<string>()): any => {
            ...
            if (s.allOf && Array.isArray(s.allOf)) {
                ...
                s.allOf.forEach((sub: any) => {
                    ...
                    if (typeof subMock === 'object' && subMock !== null) merged = {...merged, ...subMock}; else if (subMock !== null) merged = subMock;
                });
                return merged;
            }
            ...
            if ((resolvedType === 'object' || resolvedType === undefined) && s.additionalProperties && typeof s.additionalProperties === 'object') {
                const obj: any = {};
                if (s.properties) Object.entries(s.properties).forEach(([k, v]: [string, any]) => {
    ...
    const getSchemaNamesFromResponse = (resp: any): string[] => {
        ...
        Object.values(resp.content).forEach((contentObj: any) => {
            ...
            if (schema.oneOf && Array.isArray(schema.oneOf)) schema.oneOf.forEach((sub: any) => {
                if (sub.$ref) names.add(getRefName(sub.$ref));
                if (sub.title) names.add(sub.title);
            });
            if (schema.anyOf && Array.isArray(schema.anyOf)) schema.anyOf.forEach((sub: any) => {
                if (sub.$ref) names.add(getRefName(sub.$ref));
                if (sub.title) names.add(sub.title);
            });
            if (schema.allOf && Array.isArray(schema.allOf)) schema.allOf.forEach((sub: any) => {
                if (sub.$ref) names.add(getRefName(sub.$ref));
                if (sub.title) names.add(sub.title);
            });
                                            ...
                                            <tr key={idx}
                                                ...
                                                <td className="px-4 py-3 text-xs">
                                                    {param.example !== undefined ? (
                                                        <code
                                                            className="text-[10.5px] px-1.5 py-0.5 rounded bg-[var(--background)] border border-[var(--method-get)]/30 text-[var(--method-get)] font-mono select-all break-all">{String(param.example)}</code>
                                                    ) : param.schema?.example !== undefined ? (
                                                        <code
                                                            className="text-[10.5px] px-1.5 py-0.5 rounded bg-[var(--background)] border border-[var(--method-get)]/30 text-[var(--method-get)] font-mono select-all break-all">{String(param.schema.example)}</code>
                                                        ...
                                                        className="text-[var(--text-muted)] italic text-[10px]">None</span>}
                                                </td>
                                                <td className="px-4 py-3 text-xs select-none">
                                                    {param.required ? <span
                                                            className="text-[var(--method-delete)] font-bold text-xs">Yes</span> :
                                                        <span>No</span>}
                                                </td>
                                            </tr>
                                        );
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div className="flex min-w-0 items-center justify-end gap-2 flex-wrap">
                                <span
                                    ...
                                    type</span>
                                <CustomDropdown value={selectedRequestBodyContentType}
                            ...
                            return (
                                <div key={code} id={`response-${code}`}
                                    ...
                                    <div onClick={() => toggleResponse(code)}
                                        ...
                                        <div
                                            className="flex items-center gap-1.5 sm:gap-2 text-[var(--text-muted)] shrink-0">
                                            ...
                                            {resp.headers && (
                                                <div>
                                                    <p className="text-[11px] font-bold uppercase tracking-wider mb-1.5 text-[var(--text-muted)]">Response
                                                        Headers</p>
                                                    <div
                                                        className="border rounded-lg overflow-hidden border-[var(--border)]">
                                                        <div className="overflow-x-auto scrollbar-thin">
                                                            ...
                                                            return (
                                                                <div key={cType} className="space-y-3 min-w-0">
                                                                    ...
                                                                    {activeResponseTab === 'example' ? (
                                                                        <div className="space-y-3 min-w-0">
                                                                            <div
                                                                                className="pt-2 border-t border-[var(--border)] min-w-0">
                                                                            ...
                                                                            <CodeViewer
                                                                                ...
                                                                                language={getLanguageForContentType(cType)}
                                                                                maxHeight="none"/>
                                                                        </div>
