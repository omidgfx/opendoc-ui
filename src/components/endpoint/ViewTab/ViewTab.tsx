...
const getPatternFromParam = (param: any, spec: OpenApiSpec | null): string | null => {
    ...
    if (param.schema?.pattern)
        return param.schema.pattern;
    if (param.schema?.$ref) {
        const refName = getRefName(param.schema.$ref);
        const refSchema = spec?.components?.schemas?.[refName];
        if (refSchema?.pattern)
            return refSchema.pattern;
        if (refSchema?.schema?.pattern)
            return refSchema.schema.pattern;
    }
    return null;
};
export default function ViewTab({ spec, path, method, operation, onOpenSchemaModal, activeAuth, activeResponseCode, onSelectResponseCode, parsableKey = '', isActive = true, }: ViewTabProps) {
    ...
    const toggleResponse = (code: string) => {
        ...
        if (onSelectResponseCode)
            onSelectResponseCode(nextCollapsed ? null : code);
    };
    ...
    useEffect(() => {
        ...
        if (!el)
            return;
        if (initialResponseCodeRef.current)
            return;
    ...
    useEffect(() => {
        ...
        const handler = (e: KeyboardEvent) => {
            ...
            let nextIdx: number;
            if (currentIdx === -1)
                nextIdx = goingDown ? 0 : codes.length - 1;
            else
                nextIdx = goingDown ? (currentIdx + 1) % codes.length : (currentIdx - 1 + codes.length) % codes.length;
            const nextCode = codes[nextIdx];
            if (!e.shiftKey && currentCode) {
                setCollapsedResponses(prev => ({ ...prev, [currentCode]: true }));
            }
            if (onSelectResponseCode)
                onSelectResponseCode(nextCode);
            else {
    ...
    const { viewerExampleSchemas, viewerExampleNames, resolveReference, renderSchemaButton, renderSchemaTypeExample, } = useSchemaViewer(spec, isMobile, onOpenSchemaModal);
    const resolveProperties = (sObj: any, prefix = '', visited = new Set<string>()): {
        [name: string]: any;
    } => {
        if (!sObj)
            return {};
        ...
        if (sObj.$ref) {
            ...
            const refSchema = spec.components?.schemas?.[refName];
            if (refSchema)
                props = { ...props, ...resolveProperties(refSchema, prefix, visited) };
            return props;
        }
    ...
    const renderSchemaPropertiesTable = (schema: any, inspectName?: string | null) => {
        ...
        return (<SchemaPropertiesTable properties={properties} schema={schema} resolveReference={resolveReference} getRefName={getRefName} onPushSchema={onOpenSchemaModal} inspectName={inspectName ?? null} onViewExample={(name, subSchema) => setExampleModalContent({
                title: `${name} Simulated Example`,
                content: getMockSnippet(subSchema)
            })} onTestPattern={setPatternToTest} useModal={true}/>);
    };
    const { getMockSnippet, getSchemaDisplayName, getLanguageForContentType, getResponseExampleSnippet, humanizeSchemaName, getSchemaNamesFromResponse, truncateText, } = createResponseExampleHelpers(spec);
    ...
    const requestBodyContentEntries = Object.entries(resolvedRequestBody?.content || {}) as [
        string,
        any
    ][];
    ...
    return (<div ref={scrollContainerRef} className="w-full h-full overflow-y-auto p-3 sm:p-6 md:p-8 mx-auto space-y-6 sm:space-y-8 animate-in fade-in duration-200 select-text font-sans scrollbar-thin min-w-0" style={{ maxWidth: '100%' }}>
            ...
            setTimeout(() => setCopiedPath(false), 2000);
                                    ...
                                    {copiedPath ? <i className="ph ph-check text-[var(--method-get)] text-[11px]"></i> :
            <i className="ph ph-copy text-[11px]"></i>}
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        {operation.deprecated && (<span className="inline-flex items-center gap-1.5 pe-2.5 ps-1.5 py-1 text-[10px] font-bold font-sans rounded-full border bg-[var(--method-put)]/10 border-[var(--method-put)]/20 text-[var(--method-put)] select-none">
                                <i className="ph ph-warning-circle text-[16px]"></i> Deprecated
                            </span>)}
                        {isProtected && (<span className="inline-flex items-center gap-1.5 pe-2.5 ps-1.5 py-1 text-[10px] font-bold font-sans rounded-full border bg-[var(--method-delete)]/10 border-[var(--method-delete)]/20 text-[var(--method-delete)] select-none animate-pulse">
                                <i className="ph-fill ph-lock-key text-[16px]"></i> Protected
                            </span>)}
                        <Tip content="Share this endpoint">
                            <button onClick={handleShareEndpoint} className="w-7 h-7 rounded flex items-center justify-center text-xs transition-colors cursor-pointer select-none text-[var(--text-muted)] hover:text-[var(--primary)] hover:bg-[var(--primary)]/10">
                ...
                <div className="min-w-0">
                    ...
                    {operation.description && (<div className="mt-2 text-sm max-w-none text-inherit leading-relaxed animate-in fade-in text-[var(--text)]">
                            <Markdown text={operation.description}/>
                        </div>)}
                    {operation.externalDocs && operation.externalDocs.url && (<div className="mt-3">
                            <a href={operation.externalDocs.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-[var(--primary-contrast)] transition-all hover:opacity-90 cursor-pointer shadow-sm select-none bg-[var(--primary)]">
            ...
            <div className="w-full space-y-8 mx-auto min-w-0">
                {mergedParameters.length > 0 && (<div className="space-y-3 min-w-0">
                        <h2 className="text-sm font-bold uppercase tracking-wider text-[var(--text-muted)]">Request
                            Parameters</h2>
                        <div className="border rounded-2xl overflow-hidden animate-in fade-in border-[var(--border)] bg-[var(--surface)] min-w-0">
                            <div className="overflow-x-auto scrollbar-thin">
                                <table className="w-full text-left border-collapse" style={{ minWidth: 560 }}>
                                    <thead>
                                    <tr>
                                        <th className="px-4 py-3 text-xs font-semibold text-[var(--text-heading)] border-b border-[var(--border)]">Parameter
                                            Name
                                        </th>
                                        <th className="px-4 py-3 text-xs font-semibold text-[var(--text-heading)] border-b border-[var(--border)]">Location</th>
                                        <th className="px-4 py-3 text-xs font-semibold text-[var(--text-heading)] border-b border-[var(--border)]">Schema
                                            / Pattern
                                        </th>
                ...
                return (<tr key={idx} className="hover:bg-[var(--surface-hover)] transition-colors border-b border-[var(--border)]">
                                                <td className="px-4 py-3 text-xs align-top">
                                                    <div className="flex items-start flex-wrap gap-1">
                                                        <span className="font-mono font-bold text-[var(--text-heading)]">{param.name}</span>
                                                        {param.description && param.description.length > 80 && (<Tip content={param.description}>
                                                                <button onClick={() => setHelpModalContent({
                            title: param.name,
                            content: param.description || ''
                                                    ...
                                                    {param.description && (<p className="text-[10px] mt-0.5 leading-normal max-w-md break-words text-[var(--text-muted)]">{truncateText(param.description, 80)}</p>)}
                                                </td>
                                                ...
                                                <td className="px-4 py-3 text-xs">
                                                    {param.example !== undefined ? (<code className="text-[10.5px] px-1.5 py-0.5 rounded bg-[var(--background)] border border-[var(--method-get)]/30 text-[var(--method-get)] font-mono select-all break-all">{String(param.example)}</code>) : param.schema?.example !== undefined ? (<code className="text-[10.5px] px-1.5 py-0.5 rounded bg-[var(--background)] border border-[var(--method-get)]/30 text-[var(--method-get)] font-mono select-all break-all">{String(param.schema.example)}</code>) : param.schema?.default !== undefined ? (<div className="flex flex-col gap-0.5">
                                                        ...
                                                        </div>) : <span className="text-[var(--text-muted)] italic text-[10px]">None</span>}
                                                </td>
                                                <td className="px-4 py-3 text-xs select-none">
                                                    {param.required ? <span className="text-[var(--method-delete)] font-bold text-xs">Yes</span> :
                        <span>No</span>}
                                                </td>
                                            </tr>);
            })}
                ...
                {selectedRequestBodyContent && (<div className="space-y-3 font-sans min-w-0">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <h2 className="text-sm font-bold uppercase tracking-wider text-[var(--text-muted)]">Request
                                Body Context</h2>
                            <div className="flex min-w-0 items-center justify-end gap-2 flex-wrap">
                                <span className="shrink-0 text-[9px] font-black uppercase tracking-wider text-[var(--text-muted)]">Encoding
                                    type</span>
                                <CustomDropdown value={selectedRequestBodyContentType} onChange={setRequestBodyContentType} options={requestBodyContentEntries.map(([contentType]) => ({
                value: contentType,
                label: contentType
                ...
                <p className="mb-4 text-xs font-semibold leading-relaxed text-[var(--text)]">{resolvedRequestBody.description}</p>}
                            <div key={selectedRequestBodyContentType} className="space-y-4 animate-fade-in">
                                <p className="text-xs font-mono select-none">
                                    <span className="mr-1 font-sans font-semibold text-[var(--text-heading)]">Encoding
                                        TYPE:</span>
                                ...
                                <div className="pt-1 min-w-0">{renderSchemaPropertiesTable(selectedRequestBodyContent.schema, selectedRequestBodyContent.schema?.$ref
                ? getRefName(selectedRequestBodyContent.schema.$ref)
                : (selectedRequestBodyContent.schema?.title || null))}</div>
                                <div className="border-t border-[var(--border)] pt-2">
                                    <h4 className="mb-2 text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Inspect
                                        Body Schema</h4>
                ...
                <div className="space-y-3 animate-in fade-in min-w-0">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Response
                        Matrix</h4>
            ...
            const setResponseTab = (tab: 'example' | 'schema' | 'enum') => setResponseActiveTab(prev => ({
                ...prev,
                [code]: tab
            }));
            const schemaNames = getSchemaNamesFromResponse(resp);
            return (<div key={code} id={`response-${code}`} className="rounded-xl border overflow-hidden transition-all duration-150 animate-in fade-in bg-[var(--surface)] border-[var(--border)] group/resp">
                                    <div onClick={() => toggleResponse(code)} className={clsx('px-3 sm:px-5 py-3 sm:py-3.5 flex items-center justify-between cursor-pointer select-none hover:bg-[var(--text-muted)]/5 transition-colors gap-2 min-w-0')}>
                                        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1 flex-wrap">
                                            ...
                                            {!isMobile && schemaNames.length > 0 && (<span className="hidden sm:flex items-center gap-1 text-[10px] font-mono text-[var(--text-muted)] min-w-0 flex-wrap">
                                                    {schemaNames.map((name, idx) => (<React.Fragment key={name}>
                                        ...
                                        <div className="flex items-center gap-1.5 sm:gap-2 text-[var(--text-muted)] shrink-0">
                                            <Tip content="Share link to this response">
                                                <button onClick={(e) => handleShareResponse(code, resp, e)} className="w-7 h-7 rounded-md flex items-center justify-center text-[10px] hover:bg-[var(--primary)]/10 hover:text-[var(--primary)] transition-colors cursor-pointer border border-transparent hover:border-[var(--primary)]/20">
                                    ...
                                    {!isCollapsed && (<div className="p-3 sm:p-5 border-t space-y-4 animate-in fade-in border-[var(--border)] min-w-0">
                                            {resp.headers && (<div>
                                                    ...
                                                    <div className="border rounded-lg overflow-hidden border-[var(--border)]">
                                                        <div className="overflow-x-auto scrollbar-thin">
                                                            <table className="w-full text-xs text-left border-collapse" style={{ minWidth: 400 }}>
                                                                ...
                                                                <tbody>
                                                                {Object.entries(resp.headers).map(([hName, hObj]: any) => (<tr key={hName} className="border-b border-[var(--border)]">
                                            ...
                                            {resp.content && selectedContentObj ? (<>
                                                    <div className="flex items-center justify-between gap-3 flex-wrap">
                                                        <div className="flex p-0.5 rounded-lg border w-fit border-[var(--border)] bg-[var(--background)] flex-wrap">
                                                            <button onClick={() => setResponseTab('example')} className={`px-2 sm:px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${activeResponseTab === 'example' ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' : 'hover:opacity-80'}`}>
                            ...
                            return hasEnum ? (<button onClick={() => setResponseTab('enum')} className={`px-2 sm:px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${activeResponseTab === 'enum' ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' : 'hover:opacity-80'}`}>
                                                                        Enum
                                                                    </button>) : null;
                        })()}
                                                            <button onClick={() => setResponseTab('schema')} className={`px-2 sm:px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${activeResponseTab === 'schema' ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' : 'hover:opacity-80'}`}>
                                                                <span className="hidden sm:inline">Unified Schema</span>
                                                                <span className="sm:hidden">Schema</span>
                                                            </button>
                                                        </div>
                                                        {responseContentEntries.length > 1 && (<div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
                                                                <span className="text-[10px] font-bold uppercase tracking-wider shrink-0 text-[var(--text-muted)]">Format</span>
                                                                <CustomDropdown value={selectedContentType} onChange={(val) => setResponseContentTypes(prev => ({
                                ...prev,
                                [code]: val
                            ...
                            return (<div key={cType} className="space-y-3 min-w-0">
                                                                    <p className="text-[10px] font-mono select-none text-[var(--text-muted)] break-all">Content
                                                                        Type: {cType}</p>
                                                                    {activeResponseTab === 'example' ? (<div className="space-y-3 min-w-0">
                                                                            <div className="pt-2 border-t border-[var(--border)] min-w-0">
                                                                                <h4 className="text-[10px] font-bold uppercase tracking-wider mb-2 text-[var(--text-muted)]">Inspect
                                                                                    Response Schema</h4>
                                                                                <div className="flex flex-col gap-2 min-w-0">
                                                                                    <div className="min-w-0 overflow-x-auto scrollbar-thin">{renderSchemaTypeExample(cObj.schema, code)}</div>
                                                                                    {activeSchema?.description && (<div className="text-xs p-3 rounded-lg border border-[var(--primary)]/10 bg-[var(--primary)]/5 mt-1">
                                                                                            <div className="text-[10px] uppercase tracking-wider font-extrabold text-[var(--primary)] mb-1">Schema
                                                                                                Description:
                                                                                            </div>
                                                                                            <div className="markdown-body">
                                                                                                <Markdown text={activeSchema.description}/>
                                                                                            </div>
                                                                                        </div>)}
                                                                                </div>
                                                                            </div>
                                                                        ...
                                                                        </div>) : activeResponseTab === 'enum' && isEnum ? (<div className="flex flex-wrap gap-2 p-3 rounded-xl border border-[var(--border)] bg-[var(--background)]">
                                                                            {resolvedSchema.enum.map((val: any) => <span key={JSON.stringify(val)} className="px-2.5 py-1 rounded-lg text-xs font-mono border bg-[var(--surface)] border-[var(--border)] text-[var(--text)] break-all">{JSON.stringify(val)}</span>)}
                                                                            ...
                                                                            {renderSchemaPropertiesTable(activeSchema, viewerExampleNames[code] || (cObj.schema?.$ref
                                        ...
                                        : (cObj.schema?.title || null)))}
                                                ...
                                                </>) : (<p className="text-xs italic text-[11px] text-[var(--text-muted)]">Does
                                                    not return structured body payload.</p>)}
                                        </div>)}
                                </div>);
        })}
                    </div>
            ...
            <EndpointInfoModal visible={helpTransition.shouldRender && !!helpModalContent} backdropClassName={helpTransition.backdropClassName} title={helpModalContent?.title || ''} icon="ph ph-info" closeLabel="Close Help" onClose={helpTransition.requestClose} zIndex="z-[2000]">
                <div className="text-xs leading-relaxed opacity-95">{helpModalContent?.content}</div>
            </EndpointInfoModal>

            <EndpointInfoModal visible={exampleTransition.shouldRender && !!exampleModalContent} backdropClassName={exampleTransition.backdropClassName} title={exampleModalContent?.title || ''} icon="ph ph-eye" closeLabel="Close Example" onClose={exampleTransition.requestClose} zIndex="z-[3000]">
                {exampleModalContent && <CodeViewer code={exampleModalContent.content} language="json" maxHeight="none"/>}
            </EndpointInfoModal>
