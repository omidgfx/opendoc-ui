...
const getMergedParameters = (pathItem: any, operation: any, spec: OpenApiSpec | null): any[] => {
    ...
    if (operation?.parameters) {
        operation.parameters.forEach(addParam);
    }
    if (pathItem?.parameters) {
        pathItem.parameters.forEach(addParam);
    }
...
export default function ViewTab({
        ...
        if (activeResponseCode && operation.responses?.[activeResponseCode]) {
            ...
            const timer = setTimeout(() => {
                ...
                if (el) {
                    ...
                    el.classList.add('ring-2', 'ring-[var(--primary)]', 'scale-[1.01]', 'duration-500');
                    const removeHighlight = setTimeout(() => {
                        el.classList.remove('ring-2', 'ring-[var(--primary)]', 'scale-[1.01]');
                    }, 2000);
    ...
    // ESC handling for inner help modals
    ...
    const getBaseUrlWithoutResponse = () => {
        ...
        return window.location.href.split('#response-')[0];
    };

    const getEndpointShareUrl = () => getBaseUrlWithoutResponse();

    const getResponseShareUrl = (code: string) => {
        const base = getBaseUrlWithoutResponse();
        return `${base}#response-${code}`;
    };

    const handleShareEndpoint = () => {
        const url = getEndpointShareUrl();
        setShareModal({
            url,
    ...
    const handleShareResponse = (code: string, resp: any, e?: React.MouseEvent) => {
        ...
        const url = getResponseShareUrl(code);
        setShareModal({
            url,
        ...
        if (prop.$ref) {
            ...
            return (
                <button
                    ...
                    <i className="ph ph-diamonds-four text-[12px]"></i> {refName}
                </button>);
    ...
    const isSchemaActive = (sub: any, code: string, viewerSchema: any): boolean => {
        ...
        if (sub.$ref) {
            ...
            if (viewerSchema.$ref && getRefName(viewerSchema.$ref) === subRefName) {
                return true;
            }
    ...
    const renderSchemaTypeExample = (prop: any, code: string): React.ReactNode => {
        ...
        if (prop.$ref) {
            ...
            return (
                <button
                    onClick={() => setViewerExampleSchemas((prev) => ({...prev, [code]: refSchema || prop}))}
                ...
                <div className="flex flex-col gap-1.5 items-start">
                    ...
                    <div
                        ...
                        {prop.oneOf.map((sub: any, sIdx: number) => {
                            ...
                            const label = getSubLabel(sub, sIdx);
                            return (
                                <button
                                    key={sIdx}
                                    onClick={() => setViewerExampleSchemas((prev) => ({...prev, [code]: sub}))}
                                    className={`px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${
                                        isActive ?
                                            'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' :
                                            'hover:opacity-80'}`
        ...
        if (prop.type === 'array' && prop.items) {
            if (prop.items.$ref) {
                ...
                return (
                    <span className="text-xs font-sans">
                        Array&lt;
                        <button
                            onClick={() => setViewerExampleSchemas((prev) => ({
                                ...prev,
                                [code]: refSchema || prop.items
                            }))}
                            className={`px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${
                                isActive ?
                                    'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' :
            ...
            if (prop.items.oneOf || prop.items.anyOf) {
                return (
                    <span className="text-xs font-sans">
                        Array&lt;{renderSchemaTypeExample(prop.items, code)}&gt;
                    </span>);
    ...
    const getResponseExampleSnippet = (schema: any, contentObj: any, contentType: string): string => {
        ...
        if (c.includes('html')) {
            return typeof value === 'object' ?
                `<pre>${escapeXml(JSON.stringify(value, null, 2))}</pre>` :
                String(value ?? '');
        }
        if (c.includes('yaml') || c.includes('yml')) {
    ...
    const selectedRequestBodyContentType = requestBodyContentType && resolvedRequestBody?.content?.[requestBodyContentType] ?
    ...
    // Calculate if endpoint is protected (requires lock icon)
    const opSecurity = operation.security;
    const isProtected = !!(opSecurity && opSecurity.length > 0) || !!(spec?.security && spec.security.length > 0);

    return (
        <div
            className="flex-1 w-full h-full overflow-y-auto p-4 sm:p-6 md:p-8 mx-auto space-y-6 sm:space-y-8 animate-in fade-in duration-200 select-text font-sans scrollbar-thin max-w-5xl">
            {/* Description Card */}
            <div
                className="p-6 rounded-2xl border flex flex-col gap-4 shadow-sm bg-[var(--surface)] border-[var(--border)]">

                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex flex-wrap items-center gap-3">
                        <MethodBadge method={method} size="md" className="rounded-full px-3 py-1"/>
                        <div className="flex items-center gap-1.5 max-w-full">
                            <span
                                className="font-mono text-sm font-bold truncate tracking-tight break-all text-[var(--text-heading)]">
                            ...
                            <button
                                ...
                                className={clsx(
                                    'w-7 h-7 rounded flex items-center justify-center text-xs transition-colors cursor-pointer select-none',
                                    copiedPath ? 'text-[var(--method-get)]' : 'text-[var(--text-muted)]'
                                )}
                                title="Copy path URL string">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        {operation.deprecated &&
                            <span
                                ...
                                <i className="ph ph-warning-circle text-[16px]"></i> Deprecated Endpoint
                            </span>
                        }
                        {isProtected &&
                            <span
                                ...
                                <i className="ph ph-lock-key text-[16px]"></i> Protected Endpoint
                            </span>
                        }
                ...
                <div>
                    <h1 className="text-xl font-extrabold tracking-tight font-sans text-[var(--text-heading)]">

                        {operation.summary || 'Endpoint Documentations'}
                    </h1>
            ...
            <div className="w-full space-y-8 mx-auto">
                ...
                {mergedParameters.length > 0 &&
                    <div className="space-y-3">
                        ...
                        <div
                            className="border rounded-2xl overflow-hidden animate-in fade-in border-[var(--border)] bg-[var(--surface)]">
                                    ...
                                    return (
                                        <tr key={idx}
                                            ...
                                            <td className="px-4 py-3 text-xs select-none">
                                                <span
                                                    className="px-1.5 py-0.5 rounded text-[10px] font-mono border uppercase bg-[var(--background)] border-[var(--border)] text-[var(--text-muted)]">
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    {pattern &&
                                                        <div className="flex items-center gap-1 text-[10px] font-mono">
                                                            ...
                                                            <code
                                                                className="px-1 py-0.5 rounded bg-[var(--background)] text-[var(--method-put)] border border-[var(--border)] select-all text-[9.5px]">
                                                                {pattern}
                                                            </code>
                                                            <button
                                                                type="button"
                                            ...
                                            <td className="px-4 py-3 text-xs">
                                                {param.example !== undefined ?
                                                    <code
                                                        className="text-[10.5px] px-1.5 py-0.5 rounded bg-[var(--background)] border border-[var(--method-get)]/30 text-[var(--method-get)] font-mono select-all">
                                                    ...
                                                    param.schema?.example !== undefined ?
                                                        ...
                                                        param.schema?.default !== undefined ?
                                                            <div className="flex flex-col gap-0.5">
                                                                <span
                                                                    className="text-[9px] font-semibold select-none text-[var(--text-muted)]">
                                                                    Default:</span>
                                                                <code
                                                                    className="text-[10px] px-1 py-0.5 rounded bg-[var(--background)] border text-[var(--text-muted)] font-mono select-all w-fit">
                ...
                {selectedRequestBodyContent &&
                    <div className="space-y-3 font-sans">
                        ...
                        <div
                            className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 animate-in fade-in">
                            ...
                            <div key={selectedRequestBodyContentType} className="space-y-4 animate-fade-in">
                                <p className="text-xs font-mono select-none">
                                    ...
                                    <span
                                        className="rounded bg-[var(--background)] px-2 py-0.5 text-[11px] font-bold text-[var(--text-heading)]">
                                        {selectedRequestBodyContentType}
                                    </span>
                                </p>

                                <div className="pt-1">
                                    {renderSchemaPropertiesTable(selectedRequestBodyContent.schema)}
                                </div>
                ...
                <div className="space-y-3 animate-in fade-in">
                    ...
                    <div className="space-y-2">
                        {Object.entries(operation.responses).map(([code, resp]) => {
                            ...
                            const selectedContentType = responseContentTypes[code] && resp.content?.[responseContentTypes[code]] ?
                            ...
                            return (
                                <div
                                    ...
                                    <div
                                        ...
                                        className="px-5 py-3.5 flex items-center justify-between cursor-pointer group select-none hover:bg-[var(--text-muted)]/5 transition-colors">


                                        <div className="flex items-center gap-4">
                                            <span
                                                className={`font-mono text-xs font-bold px-2 py-0.5 rounded ${isSuccess ? 'bg-[var(--method-get)]/10 text-[var(--method-get)] border border-[var(--method-get)]/20' : 'bg-[var(--method-delete)]/10 text-[var(--method-delete)] border border-[var(--method-delete)]/20'}`}>
                                                {code}
                                            </span>
                                            <span
                                                className="text-xs font-semibold leading-none text-[var(--text-heading)]">
                                                    ...
                                                    <span
                                                        ...
                                                        {schemaNames.map((name, idx) =>
                                                            <React.Fragment key={name}>
                                                                ...
                                                                <span
                                                                    className="px-1.5 py-0.5 rounded border border-[var(--border)] bg-[var(--background)]">
                                        ...
                                        <div className="flex items-center gap-2 text-[var(--text-muted)]">
                                            <span
                                                className="text-[10px] opacity-0 group-hover:opacity-100 transition-opacity uppercase font-bold pr-1">
                                            ...
                                            <button
                                                ...
                                                title="Share this response link"
                                            >
                                    ...
                                    {!isCollapsed &&
                                        <div
                                            className="p-5 border-t space-y-4 animate-in fade-in border-[var(--border)]">
                                            ...
                                            {resp.content && selectedContentObj ?
                                                <>
                                                    <div className="flex items-center justify-between gap-3 flex-wrap">
                                                        <div
                                                            className="flex p-0.5 rounded-lg border w-fit border-[var(--border)] bg-[var(--background)]">


                                                            <button
                                                                onClick={() => setResponseTab('example')}
                                                                className={`px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${activeResponseTab === 'example' ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' : 'hover:opacity-80'}`}>
                                                            ...
                                                            {(() => {
                                                                ...
                                                                return hasEnum ?
                                                                    <button
                                                                        ...
                                                                        className={`px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${activeResponseTab === 'enum' ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' : 'hover:opacity-80'}`}>
                                                            ...
                                                            <button
                                                                ...
                                                                className={`px-3 py-1 rounded-md text-xs font-semibold transition-all cursor-pointer ${activeResponseTab === 'schema' ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm font-bold' : 'hover:opacity-80'}`}>
                                                        ...
                                                        {responseContentEntries.length > 1 &&
                                                            <div className="flex items-center gap-2 min-w-[230px]">
                                                                <span
                                                                    className="text-[10px] font-bold uppercase tracking-wider shrink-0 text-[var(--text-muted)]">
                                                    ...
                                                    <div className="mt-3">
                                                        {(() => {
                                                            ...
                                                            return (
                                                                <div key={cType} className="space-y-3">
                                                                    <p className="text-[10px] font-mono select-none text-[var(--text-muted)]">
                                                                        ...
                                                                        <div className="space-y-3">
                                                                            <div
                                                                                ...
                                                                                <div className="flex flex-col gap-2">
                                                                                    <div>
                                                                                        {renderSchemaTypeExample(cObj.schema, code)}
                                                                                    </div>
                                                                                    {activeSchema?.description &&
                                                                                        <div
                                                                                            className="text-xs p-3 rounded-lg border border-[var(--primary)]/10 bg-[var(--primary)]/5 mt-1">
                                                                                            <div
                                                                                                className="text-[10px] uppercase tracking-wider font-extrabold text-[var(--primary)] mb-1">
                                                                                                Schema Description:
                                                                                            </div>
                                                                                            <div
            ...
            {/* Global help overlay */}
            {helpModalContent &&
                <div className="fixed inset-0 flex items-center justify-center p-4 z-[2000] backdrop-blur-[2px]"
                    ...
                    <div
                        ...
                        <div
                            ...
                            <span className="font-bold text-sm tracking-wide text-[var(--text-heading)]">
                                <i className="ph ph-info mr-1.5 text-[var(--primary)]"></i> Description: {helpModalContent.title}
                            </span>
                            <button onClick={() => setHelpModalContent(null)}
                                    className="w-8 h-8 rounded-lg flex items-center justify-center text-sm cursor-pointer text-[var(--text-muted)]">
            ...
            {/* Local schema sub-property simulated example modal */}
            {exampleModalContent &&
            ...
            {/* Interactive regex tester modal */}
            {patternToTest &&
                <PatternTesterModal
