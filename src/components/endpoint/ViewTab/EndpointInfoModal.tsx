...
export default function EndpointInfoModal({
    ...
    return (
        <ModalPortal>
            <div
                className={`${backdropClassName} fixed inset-0 backdrop-blur-[2px] ${zIndex}`}
                style={{backgroundColor: 'rgba(0, 0, 0, 0.4)'}}
                onMouseDown={event => {
                    if (event.target === event.currentTarget) onClose();
                }}
            >
                <div className="modal-surface w-full max-w-lg rounded-2xl border flex flex-col max-h-[80vh] overflow-hidden shadow-2xl bg-[var(--surface)] border-[var(--border)]">
                    <header className="px-4 sm:px-5 py-3 sm:py-4 border-b flex items-center justify-between border-[var(--border)] bg-[var(--background)] modal-header-mobile-pad shrink-0">
                        <span className="font-bold text-sm tracking-wide text-[var(--text-heading)] truncate">
                            <i className={`${icon} mr-1.5 text-[var(--primary)]`} />
                            {title}
                        </span>
                        <Tip content="Close">
                            <button
                    ...
                    <footer className="px-4 sm:px-5 py-3 border-t text-right border-[var(--border)] bg-[var(--background)] shrink-0">
                        <button
                            onClick={onClose}
