...
function getPropertiesOfSchema(schema: any, spec: OpenApiSpec): Record<string, any> {
    ...
    if (schema.allOf) schema.allOf.forEach((sub: any) => {
        props = {...props, ...getPropertiesOfSchema(sub, spec)};
    });
...
export default function BodyEditor(props: BodyEditorProps) {
    ...
    if (!resolvedSchema) {
        return <p className="text-xs italic text-[var(--text-muted)] py-2">No body schema defined for this media
            type.</p>;
    }
                ...
                <div className="space-y-4 animate-in fade-in">
                    <div className="border border-dashed rounded-xl p-6 text-center border-[var(--border)]">
                        ...
                        <label htmlFor="examine-file-uploader"
                               className="text-xs font-semibold text-[var(--primary)] hover:underline cursor-pointer select-none">
                        ...
                        {Object.entries(propsList).map(([pName, pSchema]: [string, any]) => {
                            ...
                            const renderControl = () => {
                                if (isFileInput) {
                                    ...
                                    return (
                                        <div
                                            className="border border-dashed rounded-xl p-4 text-center border-[var(--border)] bg-[var(--background)]">
                                            <input
                                                type="file"
                                                id={inputId}
                                                onChange={(e) => {
                            ...
                            return (
                                <div key={pName} className="grid grid-cols-1 sm:grid-cols-6 gap-3 items-baseline">
                                    <span className="text-xs font-semibold truncate sm:col-span-2 md:col-span-1"
                                          title={pName}>
                                        {pName} {isRequired && <span className="text-[var(--method-delete)]">*</span>}
                                    </span>
                                    <div className="sm:col-span-4 md:col-span-5 space-y-1.5 mb-4">
                                        {resolvedP.description &&
                                            <Markdown text={resolvedP.description} className="mb-2"/>}
                                        <div
                                            className="flex flex-wrap items-center justify-between gap-1 text-[9.5px] font-mono select-none px-1">
                                            <div className="flex items-center gap-1.5 flex-wrap">
                                                ...
                                                {resolvedP.format && (
                                                    <div>format: <span
                                                        className="text-[var(--accent)] font-semibold">{resolvedP.format}</span>
                                                    </div>
                                                )}
                                                {pattern && (
                                                    <div className="flex items-center gap-1 max-w-full">
                                                        pattern: <code
                                                        className="text-[var(--method-put)] font-semibold truncate">/{pattern}/</code>
                                                        <button
                                                            type="button"
                                            ...
                                            {pattern && !isPatternValid && (
                                                <span
                                                    className="text-[var(--method-delete)] font-bold animate-pulse shrink-0">
