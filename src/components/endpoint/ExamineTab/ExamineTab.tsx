...
export default function ExamineTab({
    ...
    useEffect(() => {
        if (!bodyTypeSupportsForm(requestBodyType))
            setBodyEditorMode('raw');
    ...
    useEffect(() => {
        const handleBridgeAction = (event: Event) => {
            ...
            if (!action || (action.action !== 'set_runner_fields' && action.action !== 'run_api'))
                return;
            ...
            if (action.bodyType)
                setRequestBodyType(action.bodyType);
