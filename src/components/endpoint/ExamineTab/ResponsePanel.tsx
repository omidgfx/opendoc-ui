    ...
    return (
        <div>
            <label className="block text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Response
                Outcome</label>
            {/* Sticky so the Send/Run button stays reachable even when a long response
                pushes the rest of this panel far down the scroll area. */}
            <div className="sticky -top-8 z-20 py-3 bg-[var(--background)]/95 backdrop-blur-sm">
                <div
                    className="p-2 rounded-xl border font-mono text-xs w-full flex items-center justify-between gap-2 sm:gap-4 bg-[var(--surface)] border-[var(--border)] text-[var(--text-heading)] flex-wrap sm:flex-nowrap shadow-sm">
                    <div className="flex items-center grow gap-1.5 ps-2 min-w-0 overflow-hidden">
                        <span
                            className="uppercase font-bold opacity-60 hidden sm:inline text-[9px] tracking-widest shrink-0">URL</span>
                        <span className="opacity-85 truncate select-all text-[11px]"
                              title={requestUrl}>{requestUrl}</span>
                    </div>
                    <div className="flex items-center gap-2 shrink-0 select-none">
                        <span className="w-2 h-2 rounded-full animate-pulse"
                              style={{backgroundColor: isRunning ? '#ef4444' : '#10b981'}}></span>
                        <span
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        {isRunning ? (
                            ...
                            <Tip content="Ctrl/⌘+Enter">
                                <button type="submit" id="examine-send-btn"
                                    ...
                                    <span className="hidden sm:inline">Send API Request</span><span
                                    className="sm:hidden">Send</span><i className="ph-fill ph-play text-[18px]"></i>
                                </button>
                            </Tip>
                        )}
                    </div>
            ...
            <div className="space-y-3">
                <div
                    className="border rounded-xl overflow-hidden flex flex-col min-h-[200px] bg-[var(--surface)] border-[var(--border)]">
                    {hasResponse ? (
                        <>
                            <div
                                className="px-4 py-3 border-b flex flex-wrap items-center justify-between gap-2 border-[var(--border)] bg-[var(--background)]">
                                <div className="flex items-center gap-2">
                                    <span
                                        className="text-xs uppercase font-bold text-[var(--text-heading)]">Status:</span>
                                    <span
                            ...
                            <div className="p-4 flex-1 h-full space-y-4">
                                ...
                                <div>
                                    <label
                                        className="block text-[10px] font-bold uppercase tracking-wider mb-1.5 text-[var(--text-muted)]">Response
                                        Payload</label>
                                    <CodeViewer
                        ...
                        <div
                            className="flex-1 flex flex-col items-center justify-center p-8 text-center select-none opacity-60">
                            <span
                                className="w-12 h-12 rounded-full flex items-center justify-center text-lg mb-3 bg-[var(--background)] text-[var(--text-muted)]">
