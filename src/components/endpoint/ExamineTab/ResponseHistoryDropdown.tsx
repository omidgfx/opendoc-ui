...
const outcomeColor = (response: ExamineResponse) => {
    ...
    if (response.status >= 200 && response.status < 400) return 'bg-[var(--method-get)]';
    if (response.status >= 400) return 'bg-[var(--method-delete)]';
    ...
    const menu = open && (
        ...
        >
            <div className="flex items-center justify-between bg-[var(--background)] px-3 py-2">
                <span className="text-[9px] font-black uppercase tracking-[0.15em] text-[var(--text-muted)]">
                    Response history · {history.length}/10
                </span>
                <button
                    type="button"
                    onClick={() => {
            ...
            <div className="max-h-80 overflow-y-auto p-1.5 scrollbar-thin">
                {history.map((response, index) => (
                    ...
                    >
                        <span
                            className={`mt-1.5 size-2 shrink-0 rounded-full ${index === activeIndex ? 'bg-[var(--primary)]' : outcomeColor(response)}`}
                        />
                        <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] font-extrabold text-[var(--text-heading)]">
                                    {index === 0 ? 'Latest · ' : ''}
                        ...
                        <button
                            type="button"
                            aria-label={`Delete ${outcomeLabel(response)} from history`}
                            onClick={event => {
                                event.stopPropagation();
                                onDelete(index);
                                if (history.length <= 1) close(true);
                                else setActiveIndex(current => Math.min(current, history.length - 2));
                            }}
    ...
    return (
        <>
            ...
            >
                ...
                <span className="min-w-0 flex-1 truncate text-[10px] font-semibold text-[var(--text-heading)]">
                    {selected ? outcomeLabel(selected) : 'Response history'}
                </span>
                <i
                    className={`ph ph-caret-down text-[9px] text-[var(--text-muted)] transition-transform ${open ? 'rotate-180' : ''}`}
                />
            </button>
