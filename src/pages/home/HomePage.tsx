...
export default function HomeView({ spec, activeAuth, onSelectEndpoint, selectedEndpoint, selectedServer, onSelectServer, onDeepLinkResponse }: HomeViewProps) {
    ...
    if (!spec) {
        return (<div className="flex-1 flex flex-col items-center justify-center p-8 text-center select-none opacity-60">
    ...
    const renderSecuritySchemes = () => {
        ...
        if (!schemes || Object.keys(schemes).length === 0) {
            return (<p className="text-xs italic text-[var(--text-muted)]">
                    No security schemes are defined in this specification.
                </p>);
        }
        return (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(schemes).map(([key, config]: [
                string,
                any
                    ...
                    <p className="text-[11px] mb-2 text-[var(--text)]">{config.description}</p>}
                        <ul className="text-[10px] space-y-1 font-mono text-inherit text-[var(--text-muted)]">
                            ...
                            {config.name &&
                    <li>Param Name: <span>{config.name}</span></li>}
                            {config.in &&
                    <li>Source In: <span>{config.in}</span></li>}
                            {config.scheme &&
                    <li>HTTP Scheme: <span>{config.scheme}</span>
                                </li>}
                        </ul>
                    </div>)}
            </div>);
    };
    return (<div className="flex-1 w-full h-full overflow-y-auto p-3 sm:p-4 md:p-8 space-y-6 sm:space-y-8 animate-in fade-in duration-200 select-text font-sans scrollbar-thin min-w-0">
                ...
                <div className="relative z-10 space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <Tip content="Share this specification">
                            <button onClick={handleShareSpec} className="h-8 px-3 rounded-xl border text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer bg-[var(--primary)] text-[var(--primary-contrast)] border-[var(--primary)] hover:opacity-90 shadow-sm">
            ...
            <div className="space-y-3">
                    ...
                    <div className="pt-3 border-t mt-3 flex flex-col gap-1.5 border-[var(--border)]">
                                        ...
                                        <div>
                                            <a href={spec.externalDocs.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-[var(--primary-contrast)] transition-all hover:opacity-90 cursor-pointer shadow-sm select-none bg-[var(--primary)]">
            ...
            <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {spec.servers.map((srv, sIdx) => <div key={sIdx} className="p-4 rounded-xl border flex flex-col gap-2 shadow-sm bg-[var(--surface)] border-[var(--border)]">
