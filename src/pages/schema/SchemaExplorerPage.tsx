...
export default function SchemaExplorer({schemas = {}, onSelectSchema, parsableKey = 'API'}: SchemaExplorerProps) {
    ...
    return (<div
        ...
        <div
            ...
            <div
                className="grid w-full min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-2 min-[1320px]:w-auto min-[1320px]:grid-cols-[auto_20rem]">
                <Tip content="Export all schemas as a zip of TypeScript models" wrapperClassName="shrink-0">
                    <button onClick={() => generateAndDownloadZip(schemas as any, parsableKey)}
                ...
                <div className="relative min-w-0 w-full min-[1320px]:w-80">
                    <input ref={searchInputRef} type="text" placeholder="Search schemas (Ctrl+K)..." value={searchTerm}
        ...
        <div className="hidden md:flex items-center gap-0.5 flex-wrap shrink-0 select-none">
            <button type="button" onClick={() => setLetterFilter(null)}
                    className={clsx('px-2 h-6 rounded-md text-[10px] font-bold transition-all cursor-pointer', letterFilter === null
            ...
            {'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map(ch => {
                ...
                return (<button key={ch} type="button" disabled={!available}
                                onClick={() => setLetterFilter(letterFilter === ch ? null : ch)}
                                className={clsx('px-1.5 h-6 min-w-[18px] rounded-md text-[10px] font-bold transition-all cursor-pointer', letterFilter === ch
                                    ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm'
                                    : available
                                        ? 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-heading)]'
            ...
            <button type="button" disabled={!availableKeys.has('#')}
                    ...
                    className={clsx('px-1.5 h-6 min-w-[18px] rounded-md text-[10px] font-bold transition-all cursor-pointer', letterFilter === '#'
                        ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm'
                        : availableKeys.has('#')
                            ? 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-heading)]'
                            : 'text-[var(--text-muted)]/30 cursor-not-allowed')}>
        ...
        <div className="flex-1 min-h-0 flex flex-row">
            ...
            <div className="md:hidden min-h-0 shrink-0 self-stretch flex flex-col justify-start overflow-hidden py-1">
                <div
                    ...
                    <button type="button" onClick={() => setLetterFilter(null)}
                            className={clsx('h-5 w-6 shrink-0 rounded-md text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center', letterFilter === null ? 'bg-[var(--primary)] text-[var(--primary-contrast)]' : 'text-[var(--text-muted)]')}>
                        All
                    </button>
                    {'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map(ch => {
                        const available = availableKeys.has(ch);
                        return (<button key={ch} type="button" disabled={!available}
                                        onClick={() => setLetterFilter(letterFilter === ch ? null : ch)}
                                        className={clsx('h-5 w-6 shrink-0 rounded-md text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center', letterFilter === ch
                    ...
                    <button type="button" disabled={!availableKeys.has('#')}
                            onClick={() => setLetterFilter(letterFilter === '#' ? null : '#')}
                            className={clsx('h-5 w-6 shrink-0 rounded-md text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center', letterFilter === '#' ? 'bg-[var(--primary)] text-[var(--primary-contrast)]' : availableKeys.has('#') ? 'text-[var(--text-muted)]' : 'text-[var(--text-muted)]/30 cursor-not-allowed')}>
                        #
                    </button>
                    <button type="button" disabled={!availableKeys.has('&')}
                            onClick={() => setLetterFilter(letterFilter === '&' ? null : '&')}
                ...
                {!hasSchemas ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                        {filteredSchemas.map(([name, schema]) => {
                            ...
                            return (<div key={name} onClick={() => onSelectSchema(name)} role="button" tabIndex={0}
                                         onKeyDown={(e) => {
