...
export default function SchemaExplorer({ schemas = {}, onSelectSchema, parsableKey = 'API' }: SchemaExplorerProps) {
    ...
    return (<div className="flex-1 h-full flex flex-col p-4 md:p-8 w-full space-y-3 animate-in fade-in duration-200 select-text font-sans overflow-hidden min-w-0">
            ...
            <div className="flex flex-col min-[1320px]:flex-row min-[1320px]:items-center justify-between gap-4 border-b pb-6 shrink-0 border-[var(--border)]">
                ...
                <div className="grid w-full min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-2 min-[1320px]:w-auto min-[1320px]:grid-cols-[auto_20rem]">
                    <Tip content="Export all schemas as a zip of TypeScript models" wrapperClassName="shrink-0">
                        <button onClick={() => generateAndDownloadZip(schemas as any, parsableKey)} className="h-8 px-3 sm:px-4 rounded-lg text-xs font-bold flex items-center gap-2 transition-all cursor-pointer select-none shrink-0 bg-[var(--method-get)] text-[var(--method-get-contrast)] border-[var(--method-get)] hover:opacity-90">
                            <i className="ph ph-download-simple text-[14px]"></i>
                            <span className="hidden sm:inline">Export TS (ZIP)</span><span className="sm:hidden">TS
                            ZIP</span>
                        </button>
                    </Tip>
                    ...
                    <div className="relative min-w-0 w-full min-[1320px]:w-80">
                        <input ref={searchInputRef} type="text" placeholder="Search schemas (Ctrl+K)..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-14 h-8 text-xs rounded-lg border outline-none focus:border-[var(--primary)] transition-all font-sans bg-[var(--surface)] border-[var(--border)] text-[var(--text)]"/>
                        ...
                        {searchTerm ?
            <button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-0 pr-3 flex items-center text-xs hover:opacity-80 cursor-pointer text-[var(--text-muted)]">
            ...
            <div className="hidden md:flex items-center gap-0.5 flex-wrap shrink-0 select-none">
                <button type="button" onClick={() => setLetterFilter(null)} className={clsx('px-2 h-6 rounded-md text-[10px] font-bold transition-all cursor-pointer', letterFilter === null
            ...
            return (<button key={ch} type="button" disabled={!available} onClick={() => setLetterFilter(letterFilter === ch ? null : ch)} className={clsx('px-1.5 h-6 min-w-[18px] rounded-md text-[10px] font-bold transition-all cursor-pointer', letterFilter === ch
                    ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm'
                    : available
                        ? 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-heading)]'
                ...
                <button type="button" disabled={!availableKeys.has('#')} onClick={() => setLetterFilter(letterFilter === '#' ? null : '#')} className={clsx('px-1.5 h-6 min-w-[18px] rounded-md text-[10px] font-bold transition-all cursor-pointer', letterFilter === '#'
            ? 'bg-[var(--primary)] text-[var(--primary-contrast)] shadow-sm'
            : availableKeys.has('#')
                ? 'text-[var(--text-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-heading)]'
                : 'text-[var(--text-muted)]/30 cursor-not-allowed')}>
            ...
            <div className="flex-1 min-h-0 flex flex-row">
                ...
                <div className="md:hidden min-h-0 shrink-0 self-stretch flex flex-col justify-start overflow-hidden py-1">
                    <div className="mobile-letter-index flex h-full min-h-0 flex-col items-center gap-0.5 select-none overflow-y-scroll scrollbar-none pr-0.5">
                        <button type="button" onClick={() => setLetterFilter(null)} className={clsx('h-5 w-6 shrink-0 rounded-md text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center', letterFilter === null ? 'bg-[var(--primary)] text-[var(--primary-contrast)]' : 'text-[var(--text-muted)]')}>
                            All
                        </button>
                        {'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map(ch => {
            const available = availableKeys.has(ch);
            return (<button key={ch} type="button" disabled={!available} onClick={() => setLetterFilter(letterFilter === ch ? null : ch)} className={clsx('h-5 w-6 shrink-0 rounded-md text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center', letterFilter === ch
                    ? 'bg-[var(--primary)] text-[var(--primary-contrast)]'
                    : available ? 'text-[var(--text-muted)]' : 'text-[var(--text-muted)]/30 cursor-not-allowed')}>
                                    {ch}
                                </button>);
        })}
                        <button type="button" disabled={!availableKeys.has('#')} onClick={() => setLetterFilter(letterFilter === '#' ? null : '#')} className={clsx('h-5 w-6 shrink-0 rounded-md text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center', letterFilter === '#' ? 'bg-[var(--primary)] text-[var(--primary-contrast)]' : availableKeys.has('#') ? 'text-[var(--text-muted)]' : 'text-[var(--text-muted)]/30 cursor-not-allowed')}>
                            #
                        </button>
                        <button type="button" disabled={!availableKeys.has('&')} onClick={() => setLetterFilter(letterFilter === '&' ? null : '&')} className={clsx('h-5 w-6 shrink-0 rounded-md text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center', letterFilter === '&' ? 'bg-[var(--primary)] text-[var(--primary-contrast)]' : availableKeys.has('&') ? 'text-[var(--text-muted)]' : 'text-[var(--text-muted)]/30 cursor-not-allowed')}>
                            &amp;
                ...
                <div className="flex-1 overflow-y-auto scrollbar-thin pr-1 pb-4">
                    {!hasSchemas ? (<div className="text-center py-20 animate-in fade-in duration-200 border-[var(--border)]">
                            ...
                            <p className="text-xs mt-1 text-[var(--text-muted)]">This specification does not define any
                                schemas.</p>
                        </div>) : loading ? (<div className="text-center py-20 animate-in fade-in duration-200 border-[var(--border)]">
                            <span className="w-12 h-12 rounded-full flex items-center justify-center text-lg mx-auto mb-3 bg-[var(--background)] text-[var(--text-muted)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                    ...
                    return (<div key={name} onClick={() => onSelectSchema(name)} role="button" tabIndex={0} onKeyDown={(e) => {
                                        ...
                                        <div className="min-w-0">
                                            <div className="flex items-start justify-between gap-3 mb-2 min-w-0">
                                                <div className="min-w-0 flex-1">
                                                    <h3 className="font-medium text-xs tracking-tight transition-colors line-clamp-2 text-[var(--text-heading)] whitespace-normal break-words" title={name}>
                                                        {humanizeName(name)}
                                                    </h3>
                                                    <p className="mt-0.5 truncate text-[10px] font-mono text-[var(--text-muted)]" title={name}>
                                                        {name}
                                                    </p>
                                                </div>
                            ...
                            e.stopPropagation();
                            if (schemas)
                                generateSingleSchemaFile(name, schema, schemas as any, parsableKey);
