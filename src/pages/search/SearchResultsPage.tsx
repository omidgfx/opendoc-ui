...
export default function SearchResultsView({
    ...
    const handleShareEndpoint = (path: string, method: string, summary?: string, e?: React.MouseEvent) => {
        ...
        const url = absoluteRouteHref(
            `#/parsable/${encodeURIComponent(parsableKey)}/api/${encodeURIComponent(endpointId)}`,
        );
        setShareModal({
            url,
    ...
    const {allTags, allMethods} = useMemo(() => {
        ...
        getDocumentOperations(spec).forEach(({method, operation}) => {
            ...
            if (operation.tags && Array.isArray(operation.tags)) operation.tags.forEach(tag => tagsSet.add(tag));
    ...
    const searchResults = useMemo(() => {
        ...
        getDocumentOperations(spec).forEach(({path: pathStr, method: methodStr, operation: op}) => {
            ...
            if (query) {
                ...
                if (displayRoutes && pathLower === query) score += 100;
                else if (displayRoutes && terms.every(term => pathLower.includes(term))) score += 50;
                if (terms.every(term => summary.includes(term))) score += 30;
                if (terms.every(term => desc.includes(term))) score += 10;
                if (methodStr.toLowerCase() === query) score += 40;
                opTags.forEach(t => {
                    if (terms.every(term => t.toLowerCase().includes(term))) score += 15;
                });
            } else score = 1;
            ...
            <div className="w-full flex flex-col md:flex-row gap-6 md:gap-8 items-start min-w-0">
                <div className="w-full flex md:hidden items-center justify-between gap-2 shrink-0">
                    <p className="text-xs tracking-wide text-[var(--text-muted)] truncate">
                        Found{' '}
                        <strong className="font-semibold text-[var(--text-heading)]">{searchResults.length}</strong>{' '}
                ...
                <div className="min-w-0 flex-1 w-full">
                    ...
                    {searchResults.length > 0 ? (
                        <div className="space-y-8 sm:space-y-10 w-full max-w-full">
                            {' '}
                            {searchResults.map(({path, method, operation, isProtected, isAuthorized}) => (
                                <div key={`${method}-${path}`} className="animate-fade-in min-w-0 w-full max-w-lg">
                                    <div className="flex items-center gap-2 mb-0.5 group min-w-0 w-fit">
                                        <div
                                            className="inline-flex items-center gap-2 cursor-pointer flex-1 min-w-0"
                                        ...
                                        >
                                            ...
                                            <h3
                                                className={clsx(
                                                    `text-[15px] font-medium leading-snug group-hover:underline inline-block truncate ${operation.deprecated ? 'line-through opacity-70' : ''}`,
                                                    'text-[var(--primary)]',
                                                )}
                                            >
                                    ...
                                    <div className="text-[11px] leading-tight mb-1.5 min-w-0 text-[var(--text-muted)]">
                                        <span className="font-mono select-all block truncate">
                                            {getBreadcrumbs(path)}
                                        </span>
                                    </div>
                                    ...
                                    {operation.description ? (
                                        <div className="text-[13px] leading-relaxed line-clamp-2 text-[var(--text)] max-w-full break-words">
                                            <Markdown
                                                ...
                                                text={
                                                    operation.description.length > 240
                                                        ? `${operation.description.substring(0, 240)}...`
                                                        : operation.description
                                                }
                                            />
                                        </div>
                ...
                >
                    <div className="modal-surface w-full sm:max-w-md max-h-[85vh] rounded-t-2xl sm:rounded-2xl border shadow-2xl overflow-hidden flex flex-col bg-[var(--surface)] border-[var(--border)]">
                        <div className="px-4 py-3 border-b flex items-center justify-between shrink-0 bg-[var(--background)] border-[var(--border)] modal-header-mobile-pad">
                            ...
                            <button
                                onClick={filterTransition.requestClose}
