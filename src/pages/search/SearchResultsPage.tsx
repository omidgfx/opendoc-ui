...
export default function SearchResultsView({ spec, searchQuery, onSelectEndpoint, onMiddleClickEndpoint, selectedServer, selectedMethods, setSelectedMethods, selectedTags, setSelectedTags, onlyProtected, setOnlyProtected, displayRoutes, parsableKey = 'API' }: SearchResultsViewProps) {
    ...
    return (<div className="flex-1 w-full h-full overflow-y-auto px-3 sm:px-6 md:px-8 py-4 sm:py-6 scrollbar-thin select-text font-sans bg-[var(--surface)] min-w-0">
            <div className="w-full flex flex-col md:flex-row gap-6 md:gap-8 items-start min-w-0">
                ...
                <div className="w-full flex md:hidden items-center justify-between gap-2 shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        <Tip content="Share this search">
                            <button onClick={handleShareSearch} className="size-8 text-[10px] font-bold rounded-lg border flex items-center justify-center cursor-pointer border-[var(--border)] text-[var(--text-heading)] bg-[var(--surface)]">

                                <i className="ph ph-share-network text-[13px]"></i>
                            </button>
                        </Tip>
                        <button onClick={() => setFiltersModalOpen(true)} className="px-3 h-8 text-[10px] font-bold rounded-lg border flex items-center gap-1.5 cursor-pointer border-[var(--border)] text-[var(--text-heading)] bg-[var(--surface)]">
                ...
                <div className="min-w-0 flex-1 w-full">
                    <div className="border-b pb-4 mb-6 border-[var(--border)] hidden md:flex items-center justify-between gap-3">
                        ...
                        <Tip content="Share this search">
                            <button onClick={handleShareSearch} className="px-3 h-8 text-[10px] font-bold rounded-lg border flex items-center gap-1.5 cursor-pointer border-[var(--border)] text-[var(--text-heading)] bg-[var(--surface)]">
                                <i className="ph ph-share-network text-[12px]"></i> Share Search
                            </button>
                        </Tip>
                    </div>

                    {searchResults.length > 0 ? (<div className="space-y-8 sm:space-y-10 w-full max-w-full">                            {searchResults.map(({ path, method, operation, isProtected }) => <div key={`${method}-${path}`} className="animate-fade-in min-w-0 w-full max-w-lg">
                                <div className="flex items-center gap-2 mb-0.5 group min-w-0 w-fit">
                                    <div className="inline-flex items-center gap-2 cursor-pointer flex-1 min-w-0" onClick={() => onSelectEndpoint(path, method)} onDoubleClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    if (e.button === 1 && onMiddleClickEndpoint) {
                        ...
                        onMiddleClickEndpoint(path, method);
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                        {operation.deprecated && (<span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded border text-[9px] font-bold text-[var(--method-put)] bg-[var(--method-put)]/10 border-[var(--method-put)]/20 select-none">
                                    ...
                                    <Tip content="Share endpoint">
                                        <button onClick={(e) => handleShareEndpoint(path, method, operation.summary, e)} className="w-6 h-6 rounded-md flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-[var(--primary)]/10 hover:text-[var(--primary)] transition-all cursor-pointer shrink-0">
                                ...
                                {operation.description ? (<div className="text-[13px] leading-relaxed line-clamp-2 text-[var(--text)] max-w-full break-words">
                                        <Markdown className="markdown-body-simple" text={operation.description.length > 240 ? `${operation.description.substring(0, 240)}...` : operation.description}/>
                                ...
                                <div className="flex flex-wrap items-center gap-2 mt-2 select-none max-w-full">
                                    {operation.tags && operation.tags.map((tag) => <span key={tag} className="px-1.5 py-0.5 text-[10px] font-bold rounded uppercase border bg-[var(--background)] border-[var(--border)] text-[var(--text-muted)]">{tag}</span>)}
                                    {operation.operationId && (<span className="font-mono text-[10px] text-[var(--text-muted)] truncate">{operation.operationId}</span>)}
                                </div>
                            </div>)}
                            ...
                            <button onClick={handleClearFilters} className="mt-4 px-4 py-2 text-xs font-bold rounded-xl transition-all cursor-pointer select-none text-[var(--primary-contrast)] hover:opacity-90 shadow-sm bg-[var(--primary)]">
                                Reset All Filters
                            </button>
                        </div>)}
                </div>
                ...
                {!isMobile && (<div className="w-72 shrink-0 sticky top-0 self-start space-y-5">
                        <FiltersPanel allMethods={allMethods} allTags={allTags} selectedMethods={selectedMethods} selectedTags={selectedTags} onlyProtected={onlyProtected} handleToggleMethod={handleToggleMethod} handleToggleTag={handleToggleTag} setOnlyProtected={setOnlyProtected} handleClearFilters={handleClearFilters} hasActiveFilters={hasActiveFilters}/>
                    </div>)}
            </div>
            ...
            {filterTransition.shouldRender && isMobile && (<div className={`${filterTransition.backdropClassName} fixed inset-0 z-[2500] bg-black/50 backdrop-blur-[3px]`} onClick={(e) => {
                if (e.target === e.currentTarget)
                    filterTransition.requestClose();
            }}>
                    <div className="modal-surface w-full sm:max-w-md max-h-[85vh] rounded-t-2xl sm:rounded-2xl border shadow-2xl overflow-hidden flex flex-col bg-[var(--surface)] border-[var(--border)]">
                ...
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)]"></span>}
                            ...
                            <button onClick={filterTransition.requestClose} className="size-8 rounded-lg flex items-center justify-center hover:bg-[var(--surface-hover)] text-[var(--text-muted)]">
                        ...
                        <div className="px-4 py-3 border-t flex gap-2 shrink-0 bg-[var(--background)] border-[var(--border)]">
                            {hasActiveFilters && (<button onClick={handleClearFilters} className="flex-1 py-2 rounded-lg text-xs font-bold border cursor-pointer border-[var(--border)] text-[var(--method-delete)] hover:bg-[var(--surface-hover)]">
                                    Reset All
                                </button>)}
                            <button onClick={filterTransition.requestClose} className="flex-1 py-2 rounded-lg text-xs font-bold cursor-pointer bg-[var(--primary)] text-[var(--primary-contrast)] hover:opacity-90">
                                Apply
                            </button>
                        </div>
                    </div>
